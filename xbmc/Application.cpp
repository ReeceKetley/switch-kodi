...
#if defined(TARGET_SWITCH)
struct SwitchDebugConfig
{
  bool loaded = false;
  bool trace_enabled = true;
  bool skip_skin_color = false;
  bool skip_skin_includes = false;
  bool skip_skin_fonts = false;
  bool skip_skin_strings = false;
  bool skip_custom_windows = false;
  bool skip_wm_initialize = false;
  bool skip_gui_audio_load = false;
  bool render_flash_test = false;
  bool render_flash_only = false;
  bool pre_wm_flash_test = false;
  bool pre_wm_flash_only = false;
  bool trace_render_steps = false;
  bool skip_render_gui = false;
  bool skip_render_ex = false;
};

static bool SwitchCfgContains(const char* text, const char* key)
{
  return text != nullptr && key != nullptr && std::strstr(text, key) != nullptr;
}

static SwitchDebugConfig& GetSwitchDebugConfig()
{
  static SwitchDebugConfig cfg;
  if (cfg.loaded)
    return cfg;
  ...
  for (const char* p : paths)
  {
    ...
    if (SwitchCfgContains(buf, "trace=0")) cfg.trace_enabled = false;
    if (SwitchCfgContains(buf, "skip_skin_color=1")) cfg.skip_skin_color = true;
    if (SwitchCfgContains(buf, "skip_skin_includes=1")) cfg.skip_skin_includes = true;
    if (SwitchCfgContains(buf, "skip_skin_fonts=1")) cfg.skip_skin_fonts = true;
    if (SwitchCfgContains(buf, "skip_skin_strings=1")) cfg.skip_skin_strings = true;
    if (SwitchCfgContains(buf, "skip_custom_windows=1")) cfg.skip_custom_windows = true;
    if (SwitchCfgContains(buf, "skip_wm_initialize=1")) cfg.skip_wm_initialize = true;
    if (SwitchCfgContains(buf, "skip_gui_audio_load=1")) cfg.skip_gui_audio_load = true;
    if (SwitchCfgContains(buf, "render_flash_test=1")) cfg.render_flash_test = true;
    if (SwitchCfgContains(buf, "render_flash_only=1")) cfg.render_flash_only = true;
    if (SwitchCfgContains(buf, "pre_wm_flash_test=1")) cfg.pre_wm_flash_test = true;
    if (SwitchCfgContains(buf, "pre_wm_flash_only=1")) cfg.pre_wm_flash_only = true;
    if (SwitchCfgContains(buf, "trace_render_steps=1")) cfg.trace_render_steps = true;
    if (SwitchCfgContains(buf, "skip_render_gui=1")) cfg.skip_render_gui = true;
    if (SwitchCfgContains(buf, "skip_render_ex=1")) cfg.skip_render_ex = true;
    break;
  }
...
static void SwitchBootTrace(const char* fmt, ...)
{
  if (!GetSwitchDebugConfig().trace_enabled)
    return;

  const char* paths[] = {
    "sdmc:/switch/kodi/boottrace.txt",
    "sdmc:/switch/kodi-switch-slim-debug/boottrace.txt",
    "sdmc:/switch/kodi-switch/boottrace.txt",
    "sdmc:/switch/boottrace.txt",
    "sdmc:/boottrace.txt",
  };
...
#else
static inline void SwitchBootTrace(const char* fmt, ...) {}
#endif

#if defined(TARGET_SWITCH) || defined(__SWITCH__)
static bool SwitchRenderHeartbeat(bool logTicks, bool* beginRenderFailed)
{
  if (beginRenderFailed)
    *beginRenderFailed = false;

  if (!CServiceBroker::GetRenderSystem()->BeginRender())
  {
    if (beginRenderFailed)
      *beginRenderFailed = true;
    return false;
  }

  static uint64_t s_flashFrame = 0;
  ++s_flashFrame;

  const unsigned int phase = static_cast<unsigned int>((s_flashFrame / 30ULL) % 3ULL);
  unsigned int flashColor = 0xFF101010;
  if (phase == 0)
    flashColor = 0xFFFF0000; // red
  else if (phase == 1)
    flashColor = 0xFF00FF00; // green
  else
    flashColor = 0xFF0000FF; // blue

  CServiceBroker::GetWinSystem()->GetGfxContext().Clear(flashColor);
  ...
  if (logTicks && (s_flashFrame % 120ULL) == 0ULL)
  {
    CLog::Log(LOGNOTICE, "SWITCH_RENDER: heartbeat frame={} color=0x{:08x}",
              static_cast<unsigned long long>(s_flashFrame), flashColor);
  }
...
{
  SwitchBootTrace("Create: enter");
  // Grab a handle to our thread to be used later in identifying the render thread.
  m_threadID = CThread::GetCurrentThreadId();
  SwitchBootTrace("Create: thread id captured");

  m_bPlatformDirectories = params.m_platformDirectories;
  m_bTestMode = params.m_testmode;
  m_bStandalone = params.m_standAlone;
  SwitchBootTrace("Create: params copied");

  m_pSettingsComponent.reset(new CSettingsComponent());
  SwitchBootTrace("Create: settings component allocated");
  m_pSettingsComponent->Init(params);
  SwitchBootTrace("Create: settings component initialized");

  // Announement service
  m_pAnnouncementManager = std::make_shared<ANNOUNCEMENT::CAnnouncementManager>();
  SwitchBootTrace("Create: announcement manager allocated");
  m_pAnnouncementManager->Start();
  SwitchBootTrace("Create: announcement manager started");
  CServiceBroker::RegisterAnnouncementManager(m_pAnnouncementManager);
  SwitchBootTrace("Create: announcement manager registered");

  m_ServiceManager.reset(new CServiceManager());
  SwitchBootTrace("Create: service manager allocated");

  if (!m_ServiceManager->InitStageOne())
  {
    SwitchBootTrace("Create: InitStageOne failed");
    return false;
  }
  SwitchBootTrace("Create: InitStageOne ok");
  ...
  //! @todo - move to CPlatformXXX
    ...
    if (getenv("HOME"))
    {
      ...
      SwitchBootTrace("Create: HOME set from env");
    }
#if defined(TARGET_SWITCH)
    else
    {
      // Homebrew environments may not define HOME.
      // Use the already-initialized special://home path as fallback to keep
      // envhome/home/masterprofile in sync and avoid hardcoded app folders.
      std::string switchHome = CSpecialProtocol::TranslatePath("special://home/");
      if (switchHome.empty())
        switchHome = "sdmc:/switch/kodi";
      setenv("HOME", switchHome.c_str(), 1);
      CSpecialProtocol::SetEnvHomePath(switchHome.c_str());
      SwitchBootTrace("Create: HOME fallback applied");
    }
#endif
#if !defined(TARGET_SWITCH)
    else
  ...
  if (!CLog::Init(CSpecialProtocol::TranslatePath("special://logpath").c_str()))
  {
    ...
    SwitchBootTrace("Create: CLog::Init failed");
    return false;
  }
  SwitchBootTrace("Create: CLog::Init ok");
...
//  specialVersion = " (version for XXXX)";
  ...
  {
    SwitchBootTrace("Create: settings load failed");
    return false;
  }
  SwitchBootTrace("Create: settings loaded");
  ...
  if (!m_pWinSystem)
  {
    fprintf(stderr, "CreateWinSystem() returned null\n");
    SwitchBootTrace("Create: CreateWinSystem returned null");
    return false;
  }
  SwitchBootTrace("Create: CreateWinSystem ok");
  ...
  {
    SwitchBootTrace("Create: InitStageTwo failed");
    return false;
  }
  SwitchBootTrace("Create: InitStageTwo ok");

#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  // Spike mode: audio engine currently enters a tight signal loop under Switch homebrew runtimes.
  SwitchBootTrace("Create: audio engine skipped");
#else
  ...
  SwitchBootTrace("Create: replay gain settings initialized");

  // load the keyboard layouts
  SwitchBootTrace("Create: keyboard layout load begin");
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  SwitchBootTrace("Create: keyboard layout load skipped");
#else
  if (!CKeyboardLayoutManager::GetInstance().Load())
  {
    CLog::Log(LOGFATAL, "CApplication::Create: Unable to load keyboard layouts");
    SwitchBootTrace("Create: keyboard layout load failed");
    return false;
  }
  SwitchBootTrace("Create: keyboard layout loaded");
#endif
  ...
  SwitchBootTrace("Create: media manager initialized");

  m_lastRenderTime = XbmcThreads::SystemClockMillis();
  SwitchBootTrace("Create: success");
  return true;
}

bool CApplication::CreateGUI()
{
  SwitchBootTrace("CreateGUI: enter");
  ...
  if (!CServiceBroker::GetWinSystem()->InitWindowSystem())
  {
    ...
    SwitchBootTrace("CreateGUI: InitWindowSystem failed");
    return false;
  }
  SwitchBootTrace("CreateGUI: InitWindowSystem ok");

  // Retrieve the matching resolution based on GUI settings
  bool sav_res = false;
  SwitchBootTrace("CreateGUI: set current resolution begin");
  CDisplaySettings::GetInstance().SetCurrentResolution(CDisplaySettings::GetInstance().GetDisplayResolution());
  SwitchBootTrace("CreateGUI: set current resolution done");
  ...
  const std::shared_ptr<CSettings> settings = CServiceBroker::GetSettingsComponent()->GetSettings();
  SwitchBootTrace("CreateGUI: SetWindowResolution begin");
  ...
  SwitchBootTrace("CreateGUI: SetWindowResolution done");
  ...
  if (!InitWindow())
  {
    SwitchBootTrace("CreateGUI: InitWindow failed");
    return false;
  }
  SwitchBootTrace("CreateGUI: InitWindow ok");

  // Set default screen saver mode
  SwitchBootTrace("CreateGUI: screensaver setting begin");
  ...
  SwitchBootTrace("CreateGUI: screensaver setting done");

  if (sav_res)
  {
    SwitchBootTrace("CreateGUI: save safe resolution begin");
    CDisplaySettings::GetInstance().SetCurrentResolution(RES_DESKTOP, true);
    SwitchBootTrace("CreateGUI: save safe resolution done");
  }

  SwitchBootTrace("CreateGUI: GUI component alloc begin");
  m_pGUI.reset(new CGUIComponent());
  SwitchBootTrace("CreateGUI: GUI component alloc done");
  SwitchBootTrace("CreateGUI: GUI component init begin");
  m_pGUI->Init();
  SwitchBootTrace("CreateGUI: GUI component init done");

  // Splash currently disabled during switch spike bring-up.
  SwitchBootTrace("CreateGUI: splash disabled");
  SwitchBootTrace("CreateGUI: splash bypass done");

  // The key mappings may already have been loaded by a peripheral
  CLog::Log(LOGINFO, "load keymapping");
  if (!CServiceBroker::GetInputManager().LoadKeymaps())
  {
    SwitchBootTrace("CreateGUI: keymaps load failed");
    return false;
  }
  SwitchBootTrace("CreateGUI: keymaps loaded");
  ...
  CLog::Log(LOGINFO, "GUI format %ix%i, Display %s",
            ...
            info.iHeight,
            info.strMode.c_str());
  SwitchBootTrace("CreateGUI: success");
...
{
  SwitchBootTrace("InitWindow: enter");
  if (res == RES_INVALID)
                                                      ...
                                                      bFullScreen, CDisplaySettings::GetInstance().GetResolutionInfo(res)))
  {
    ...
    SwitchBootTrace("InitWindow: CreateNewWindow failed");
    return false;
  }
  SwitchBootTrace("InitWindow: CreateNewWindow ok");

  if (!CServiceBroker::GetRenderSystem()->InitRenderSystem())
  {
    CLog::Log(LOGFATAL, "CApplication::Create: Unable to init rendering system");
    SwitchBootTrace("InitWindow: InitRenderSystem failed");
    return false;
  }
  SwitchBootTrace("InitWindow: InitRenderSystem ok");
  // set GUI res and force the clear of the screen
  SwitchBootTrace("InitWindow: SetVideoResolution begin");
  CServiceBroker::GetWinSystem()->GetGfxContext().SetVideoResolution(res, false);
  SwitchBootTrace("InitWindow: SetVideoResolution done");
  return true;
}
...
bool CApplication::Initialize()
{
  ...
  SwitchBootTrace("Initialize: LoadLanguage begin");
  if (!LoadLanguage(false))
  {
    SwitchBootTrace("Initialize: LoadLanguage failed");
    return false;
  }
  SwitchBootTrace("Initialize: LoadLanguage ok");
  ...
  CEvent event(true);
  SwitchBootTrace("Initialize: db init submit");
  ...
  SwitchBootTrace("Initialize: db init done");
#if !defined(TARGET_SWITCH)
  CServiceBroker::GetRenderSystem()->ShowSplash("");
#endif

  SwitchBootTrace("Initialize: StartServices begin");
  StartServices();
  SwitchBootTrace("Initialize: StartServices done");
  ...
  if (CServiceBroker::GetGUI()->GetWindowManager().Initialized())
  {
    ...
    SwitchBootTrace("Initialize: LoadSkin begin");
    if (!LoadSkin(settings->GetString(CSettings::SETTING_LOOKANDFEEL_SKIN)))
    {
      ...
      if (!LoadSkin(defaultSkin))
      {
        ...
        SwitchBootTrace("Initialize: LoadSkin failed");
        return false;
      }
    }
    SwitchBootTrace("Initialize: LoadSkin done");
...
#if !defined(TARGET_SWITCH)
    SwitchBootTrace("Initialize: ActivateWindow splash begin");
    CServiceBroker::GetGUI()->GetWindowManager().ActivateWindow(WINDOW_SPLASH);
    SwitchBootTrace("Initialize: ActivateWindow splash done");
#else
    SwitchBootTrace("Initialize: ActivateWindow splash skipped");
#endif
    ...
    {
      SwitchBootTrace("Initialize: ActivateWindow login");
    ...
    else
    {
      ...
      {
        SwitchBootTrace("Initialize: forcing WINDOW_HOME instead of startup animation");
        firstWindow = WINDOW_HOME;
      }
#endif
      SwitchBootTrace("Initialize: ActivateWindow first begin");
      CServiceBroker::GetGUI()->GetWindowManager().ActivateWindow(firstWindow);
      SwitchBootTrace("Initialize: ActivateWindow first done");
  ...
  {
    SwitchBootTrace("Initialize: GUI window manager not initialized");
    uiInitializationFinished = true;
  }

  SwitchBootTrace("Initialize: JSONRPC begin");
  CJSONRPC::Initialize();
  SwitchBootTrace("Initialize: JSONRPC done");

  if (!m_ServiceManager->InitStageThree(profileManager))
  {
    CLog::Log(LOGERROR, "Application - Init3 failed");
    SwitchBootTrace("Initialize: InitStageThree failed");
  }
  else
  {
    SwitchBootTrace("Initialize: InitStageThree ok");
  }
  ...
  SwitchBootTrace("Initialize: done");
...
{
  SwitchBootTrace("LoadSkin: enter skinID=%s", skinID.c_str());
  SkinPtr skin;
  {
    AddonPtr addon;
    if (!CServiceBroker::GetAddonMgr().GetAddon(skinID, addon, ADDON_SKIN))
    {
      SwitchBootTrace("LoadSkin: GetAddon failed");
      return false;
    }
    skin = std::static_pointer_cast<ADDON::CSkinInfo>(addon);
  }
  SwitchBootTrace("LoadSkin: addon resolved id=%s path=%s", skin->ID().c_str(), skin->Path().c_str());
  ...
  UnloadSkin();
  SwitchBootTrace("LoadSkin: UnloadSkin done");

  skin->Start();
  SwitchBootTrace("LoadSkin: skin->Start done");
  ...
  SwitchBootTrace("LoadSkin: MigrateSettings done");
  ...
  if (!skin->HasSkinFile("Home.xml"))
  {
    ...
    SwitchBootTrace("LoadSkin: Home.xml missing");
    return false;
  }
  SwitchBootTrace("LoadSkin: Home.xml present");

  CLog::Log(LOGNOTICE, "  load skin from: %s (version: %s)", skin->Path().c_str(), skin->Version().asString().c_str());
  g_SkinInfo = skin;

  CLog::Log(LOGINFO, "  load fonts for skin...");
  std::string skinBasePath = skin->Path();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (StringUtils::EndsWithNoCase(skinBasePath, ".zip"))
  {
    const std::string unpackedPath = URIUtils::AddFileToFolder("special://home/addons", skin->ID());
    if (XFILE::CDirectory::Exists(unpackedPath))
      skinBasePath = unpackedPath;
  }
#endif
  CServiceBroker::GetWinSystem()->GetGfxContext().SetMediaDir(skinBasePath);
  SwitchBootTrace("LoadSkin: SetMediaDir=%s", skinBasePath.c_str());
  g_directoryCache.ClearSubPaths(skinBasePath);
  SwitchBootTrace("LoadSkin: ClearSubPaths done");

  const std::shared_ptr<CSettings> settings = CServiceBroker::GetSettingsComponent()->GetSettings();
  if (!GetSwitchDebugConfig().skip_skin_color)
  {
    SwitchBootTrace("LoadSkin: ColorManager.Load begin");
    CServiceBroker::GetGUI()->GetColorManager().Load(settings->GetString(CSettings::SETTING_LOOKANDFEEL_SKINCOLORS));
    SwitchBootTrace("LoadSkin: ColorManager.Load done");
  }
  else
  {
    SwitchBootTrace("LoadSkin: ColorManager.Load skipped");
  }

  if (!GetSwitchDebugConfig().skip_skin_includes)
  {
    SwitchBootTrace("LoadSkin: LoadIncludes begin");
    g_SkinInfo->LoadIncludes();
    SwitchBootTrace("LoadSkin: LoadIncludes done");
  }
  else
  {
    SwitchBootTrace("LoadSkin: LoadIncludes skipped");
  }

  if (!GetSwitchDebugConfig().skip_skin_fonts)
  {
    SwitchBootTrace("LoadSkin: LoadFonts begin");
    g_fontManager.LoadFonts(settings->GetString(CSettings::SETTING_LOOKANDFEEL_FONT));
    SwitchBootTrace("LoadSkin: LoadFonts done");
  }
  else
  {
    SwitchBootTrace("LoadSkin: LoadFonts skipped");
  }

  // load in the skin strings
  std::string langPath = URIUtils::AddFileToFolder(skinBasePath, "language");
  URIUtils::AddSlashAtEnd(langPath);

  if (!GetSwitchDebugConfig().skip_skin_strings)
  {
    SwitchBootTrace("LoadSkin: LoadSkinStrings begin");
    g_localizeStrings.LoadSkinStrings(langPath, settings->GetString(CSettings::SETTING_LOCALE_LANGUAGE));
    SwitchBootTrace("LoadSkin: LoadSkinStrings done");
  }
  else
  {
    SwitchBootTrace("LoadSkin: LoadSkinStrings skipped");
  }
  ...
  // Load custom windows
  if (!GetSwitchDebugConfig().skip_custom_windows)
  {
    SwitchBootTrace("LoadSkin: LoadCustomWindows begin");
    LoadCustomWindows();
    SwitchBootTrace("LoadSkin: LoadCustomWindows done");
  }
  else
  {
    SwitchBootTrace("LoadSkin: LoadCustomWindows skipped");
  }
  ...
  if (!GetSwitchDebugConfig().skip_wm_initialize)
  {
    SwitchBootTrace("LoadSkin: WindowManager.Initialize begin");
    CServiceBroker::GetGUI()->GetWindowManager().Initialize();
    SwitchBootTrace("LoadSkin: WindowManager.Initialize done");
  }
  else
  {
    SwitchBootTrace("LoadSkin: WindowManager.Initialize skipped");
  }
  CTextureCache::GetInstance().Initialize();
  SwitchBootTrace("LoadSkin: TextureCache.Initialize done");
  CServiceBroker::GetGUI()->GetAudioManager().Enable(true);
  SwitchBootTrace("LoadSkin: GUIAudio.Enable done");
  if (!GetSwitchDebugConfig().skip_gui_audio_load)
  {
    SwitchBootTrace("LoadSkin: GUIAudio.Load begin");
    CServiceBroker::GetGUI()->GetAudioManager().Load();
    SwitchBootTrace("LoadSkin: GUIAudio.Load done");
  }
  else
  {
    SwitchBootTrace("LoadSkin: GUIAudio.Load skipped");
  }
  ...
  if (g_SkinInfo->HasSkinFile("DialogFullScreenInfo.xml"))
  {
    ...
    SwitchBootTrace("LoadSkin: Add DialogFullScreenInfo done");
  }
  else
  {
    SwitchBootTrace("LoadSkin: DialogFullScreenInfo missing");
  }

  CLog::Log(LOGINFO, "  skin loaded...");
  SwitchBootTrace("LoadSkin: success");

  // leave the graphics lock
  SwitchBootTrace("LoadSkin: lock leave begin");
  lock.Leave();
  SwitchBootTrace("LoadSkin: lock leave done");

  // restore active window
  SwitchBootTrace("LoadSkin: restore active window begin current=%d focus=%d", currentWindowID, currentFocusedControlID);
  if (currentWindowID != WINDOW_INVALID)
  {
  ...
  SwitchBootTrace("LoadSkin: restore active window done");

  // restore player and rendering state
  SwitchBootTrace("LoadSkin: restore player/render state begin");
  if (m_appPlayer.IsPlayingVideo())
  {
    if (bPreviousPlayingState)
      m_appPlayer.Pause();
...
void CApplication::Render()
{
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  const auto& switchCfg = GetSwitchDebugConfig();
  if (switchCfg.trace_render_steps)
    CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: Render enter");
  static uint64_t s_renderCallCount = 0;
  ++s_renderCallCount;
  if ((s_renderCallCount % 120) == 0)
  {
    CLog::Log(LOGNOTICE, "SWITCH_RENDER: call={} renderGUI={} skipGui={} focused={}",
              static_cast<unsigned long long>(s_renderCallCount),
              m_renderGUI ? 1 : 0,
              m_skipGuiRender ? 1 : 0,
              m_AppFocused ? 1 : 0);
  }
  ...
  if (switchCfg.trace_render_steps)
    CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: BeginRender call");
  if(!CServiceBroker::GetRenderSystem()->BeginRender())
  {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    CLog::Log(LOGWARNING, "SWITCH_RENDER: BeginRender returned false");
#endif
    return;
  }
  if (switchCfg.trace_render_steps)
    CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: BeginRender ok");

#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (switchCfg.render_flash_test)
  {
    static uint64_t s_flashFrame = 0;
    ++s_flashFrame;
    const unsigned int phase = static_cast<unsigned int>((s_flashFrame / 30ULL) % 3ULL);
    ...
    if ((s_flashFrame % 120ULL) == 0ULL)
      CLog::Log(LOGNOTICE, "SWITCH_RENDER: flash frame={} color=0x{:08x} only={}",
                static_cast<unsigned long long>(s_flashFrame), flashColor,
                switchCfg.render_flash_only ? 1 : 0);

    if (switchCfg.render_flash_only)
    {
      CServiceBroker::GetRenderSystem()->EndRender();
      CServiceBroker::GetWinSystem()->GetGfxContext().Flip(true, false);
      CTimeUtils::UpdateFrameTime(true);
  ...
  if (switchCfg.trace_render_steps)
    CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: pre-gui renderGUI={} skipGuiRender={} skip_render_gui={}",
              m_renderGUI ? 1 : 0, m_skipGuiRender ? 1 : 0, switchCfg.skip_render_gui ? 1 : 0);

  if (m_renderGUI && !m_skipGuiRender && !switchCfg.skip_render_gui)
  {
    ...
    {
      if (switchCfg.trace_render_steps)
        CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: WindowManager.Render begin stereo=0");
      hasRendered |= CServiceBroker::GetGUI()->GetWindowManager().Render();
      if (switchCfg.trace_render_steps)
        CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: WindowManager.Render done stereo=0 hasRendered={}", hasRendered ? 1 : 0);
    }
    // execute post rendering actions (finalize window closing)
    if (switchCfg.trace_render_steps)
      CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: WindowManager.AfterRender begin");
    CServiceBroker::GetGUI()->GetWindowManager().AfterRender();
    if (switchCfg.trace_render_steps)
      CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: WindowManager.AfterRender done");
  ...
  // render video layer
  if (switchCfg.trace_render_steps)
    CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: WindowManager.RenderEx begin skip_render_ex={}", switchCfg.skip_render_ex ? 1 : 0);
  if (!switchCfg.skip_render_ex)
  ...
  CServiceBroker::GetRenderSystem()->EndRender();
  if (switchCfg.trace_render_steps)
    CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: EndRender done");
  ...
  if (switchCfg.trace_render_steps)
    CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: Flip begin hasRendered={} videoLayer={}",
              hasRendered ? 1 : 0, m_appPlayer.IsRenderingVideoLayer() ? 1 : 0);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  static uint64_t s_switchPostFlipTrace = 0;
  const bool postFlipTrace = (s_switchPostFlipTrace < 8);
  if (postFlipTrace)
    SwitchBootTrace("RENDER_POST: pre-flip hasRendered=%d videoLayer=%d",
                    hasRendered ? 1 : 0, m_appPlayer.IsRenderingVideoLayer() ? 1 : 0);
#endif
...
#endif
  if (switchCfg.trace_render_steps)
    CLog::Log(LOGNOTICE, "SWITCH_RENDER_STEP: Flip done");
...
void CApplication::FrameMove(bool processEvents, bool processGUI)
{
  ...
  if (processEvents)
  {
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (fmTrace)
      CLog::Log(LOGNOTICE, "SWITCH_FM: HandlePortEvents begin");
#endif
    HandlePortEvents();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (fmTrace)
      CLog::Log(LOGNOTICE, "SWITCH_FM: HandlePortEvents done");
#endif

#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (fmTrace)
      CLog::Log(LOGNOTICE, "SWITCH_FM: InputManager::Process begin");
#endif
    CServiceBroker::GetInputManager().Process(CServiceBroker::GetGUI()->GetWindowManager().GetActiveWindowOrDialog(), frameTime);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (fmTrace)
      CLog::Log(LOGNOTICE, "SWITCH_FM: InputManager::Process done");
#endif

    if (processGUI && m_renderGUI)
    {
      if (fmTrace)
        CLog::Log(LOGNOTICE, "SWITCH_FM: Inertial/Seek begin");
      m_pInertialScrollingHandler->ProcessInertialScroll(frameTime);
      m_appPlayer.GetSeekHandler().FrameMove();
      if (fmTrace)
        CLog::Log(LOGNOTICE, "SWITCH_FM: Inertial/Seek done");
    }
    ...
    if (m_WaitingExternalCalls)
    {
      ...
      if (fmTrace)
        CLog::Log(LOGNOTICE, "SWITCH_FM: external sleep {}ms", sleepTime);
      Sleep(sleepTime);
      m_frameMoveGuard.lock();
  ...
  if (processGUI && m_renderGUI)
  {
    ...
    if (fmTrace)
      CLog::Log(LOGNOTICE, "SWITCH_FM: pre-refresh-check");
    if (CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_guiSmartRedraw && m_guiRefreshTimer.IsTimePast())
    {
      if (fmTrace)
        CLog::Log(LOGNOTICE, "SWITCH_FM: refresh send begin");
      CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_REFRESH_TIMER, 0, 0);
      if (fmTrace)
        CLog::Log(LOGNOTICE, "SWITCH_FM: refresh send done");
      m_guiRefreshTimer.Set(500);
      if (fmTrace)
        CLog::Log(LOGNOTICE, "SWITCH_FM: refresh timer set done");
    }
    if (fmTrace)
      CLog::Log(LOGNOTICE, "SWITCH_FM: pre-wm-process-check stop={}", m_bStop ? 1 : 0);

#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    bool skipWmProcessForFlash = false;
    const auto& switchCfg = GetSwitchDebugConfig();
    if (switchCfg.pre_wm_flash_test)
    {
      bool beginRenderFailed = false;
      const bool flashed = SwitchRenderHeartbeat(true, &beginRenderFailed);
      if (beginRenderFailed)
        CLog::Log(LOGWARNING, "SWITCH_RENDER: pre_wm BeginRender returned false");
      if (switchCfg.pre_wm_flash_only)
      {
        skipWmProcessForFlash = true;
        if (fmTrace)
          CLog::Log(LOGNOTICE, "SWITCH_FM: pre_wm_flash_only active flashed={}", flashed ? 1 : 0);
      }
    }
#endif
    ...
    if (!m_bStop)
    {
      ...
      {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
        if (skipWmProcessForFlash)
        {
          if (fmTrace)
            CLog::Log(LOGNOTICE, "SWITCH_FM: WM.Process skipped by pre_wm_flash_only");
        }
        else
        {
#endif
        if (fmTrace)
          CLog::Log(LOGNOTICE, "SWITCH_FM: WM.Process begin");
        CServiceBroker::GetGUI()->GetWindowManager().Process(CTimeUtils::GetFrameTime());
        if (fmTrace)
          CLog::Log(LOGNOTICE, "SWITCH_FM: WM.Process done");
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
        }
#endif
      }
  ...
  if (fmTrace)
    CLog::Log(LOGNOTICE, "SWITCH_FM: AppPlayer.FrameMove done");

  // this will go away when render systems gets its own thread
  if (fmTrace)
    CLog::Log(LOGNOTICE, "SWITCH_FM: DriveRenderLoop begin");
...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (m_renderGUI != renderGUI)
  {
    CLog::Log(LOGNOTICE, "SWITCH_RENDER: SetRenderGUI {} -> {}", m_renderGUI ? 1 : 0, renderGUI ? 1 : 0);
  }
#endif
