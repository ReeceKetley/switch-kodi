...
bool CAddonMgr::Init()
{
  ...
  assert(m_cp_context);
  auto registerCollection = [this](const char* specialPath) -> bool
  {
    std::string translated = CSpecialProtocol::TranslatePath(specialPath);
    cp_status_t status = cp_register_pcollection(m_cp_context, translated.c_str());
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (StringUtils::StartsWith(translated, "sdmc:/"))
    {
      std::string fallbackPath = translated.substr(5); // "/switch/..."
      cp_status_t fallbackStatus = cp_register_pcollection(m_cp_context, fallbackPath.c_str());
      CLog::Log(LOGNOTICE,
                "ADDONS: register pcollection {} status={} fallback {} status={}",
                translated, static_cast<int>(status), fallbackPath, static_cast<int>(fallbackStatus));
      if (status == CP_OK || fallbackStatus == CP_OK)
        return true;
      status = fallbackStatus;
    }
    else if (status == CP_OK)
    {
      return true;
    }
#else
    ...
    CLog::Log(LOGERROR, "ADDONS: Fatal Error, cp_register_pcollection({}) returned status: {}",
              translated.c_str(), status);
    return false;
  };

  if (!registerCollection("special://home/addons"))
    return false;

  if (!registerCollection("special://xbmc/addons"))
    return false;

  if (!registerCollection("special://xbmcbin/addons"))
    return false;

  status = cp_register_logger(m_cp_context, cp_logger, this, CP_LOG_DEBUG);
  if (status != CP_OK)
  {
  ...
  if (!m_database.Open())
  {
    ...
    CLog::Log(LOGWARNING, "ADDONS: Failed to open database, continuing in scan-only mode on Switch");
#else
  ...
  for (const auto& id : m_systemAddons)
  {
    ...
    {
#if defined(TARGET_SWITCH)
      CLog::Log(LOGWARNING, "addon '%s' not installed or not enabled on Switch bootstrap; continuing", id.c_str());
      continue;
#endif
...
bool CAddonMgr::FindAddons()
{
  ...
  {
    result = true;
    const std::string homeAddons = CSpecialProtocol::TranslatePath("special://home/addons");
    CFileItemList addonDirs;
    const bool hasHomeAddonsDir = CDirectory::Exists(homeAddons);
    const bool listedHomeAddons = CDirectory::GetDirectory(homeAddons, addonDirs, "", DIR_FLAG_DEFAULTS);
    CLog::Log(LOGNOTICE, "ADDONS: pre-scan home={} exists={} listed={} items={}",
              homeAddons, hasHomeAddonsDir ? 1 : 0, listedHomeAddons ? 1 : 0, addonDirs.Size());
    for (int i = 0; i < addonDirs.Size() && i < 5; ++i)
    {
      const auto& item = addonDirs.Get(i);
      if (!item)
      {
        CLog::Log(LOGNOTICE, "ADDONS: probe item[{}]=<null>", i);
        continue;
      }
      const std::string probePath = item->GetPath();
      const std::string descriptorPath = URIUtils::AddFileToFolder(probePath, "addon.xml");
      CLog::Log(LOGNOTICE, "ADDONS: probe item[{}] path={} folder={} addon.xml_exists={}",
                i, probePath, item->m_bIsFolder ? 1 : 0, CFile::Exists(descriptorPath) ? 1 : 0);
#if defined(TARGET_SWITCH)
      if (item->m_bIsFolder)
      {
        AddonPtr probeAddon;
        const bool parsed = LoadAddonDescription(probePath, probeAddon);
        CLog::Log(LOGNOTICE, "ADDONS: probe parse item[{}] parsed={} addon_id={}",
                  i, parsed ? 1 : 0, probeAddon ? probeAddon->ID() : "<none>");
      }
#endif
    }
    const cp_status_t scanStatus = cp_scan_plugins(m_cp_context, CP_SP_UPGRADE);
    CLog::Log(LOGNOTICE, "ADDONS: cp_scan_plugins status={}", static_cast<int>(scanStatus));

    std::set<std::pair<std::string, std::string>> installed;
    auto collectInstalled = [this, &installed]() -> int
    {
      cp_status_t status;
      int n = 0;
      cp_plugin_info_t** cp_addons = cp_get_plugins_info(m_cp_context, &status, &n);
      CLog::Log(LOGNOTICE, "ADDONS: cp_get_plugins_info status={} count={}", static_cast<int>(status), n);
      for (int i = 0; i < n; ++i)
      {
    ...
    int n = collectInstalled();

#if defined(TARGET_SWITCH)
    if (n == 0)
    {
      CLog::Log(LOGWARNING, "ADDONS: cp_scan discovered no plugins on Switch; trying manual install fallback");
      for (int i = 0; i < addonDirs.Size(); ++i)
      {
        const auto& item = addonDirs.Get(i);
        if (!item || !item->m_bIsFolder)
          continue;

        const std::string addonPath = item->GetPath();
        const std::string addonXmlPath = CSpecialProtocol::TranslatePath(URIUtils::AddFileToFolder(addonPath, "addon.xml"));
        if (!CFile::Exists(addonXmlPath))
          continue;
        ...
        cp_status_t loadStatus;
        cp_plugin_info_t* info = cp_load_plugin_descriptor_from_memory(m_cp_context, buffer.get(), buffer.size(), &loadStatus);
        if (!info)
        {
          CLog::Log(LOGWARNING, "ADDONS: manual parse failed path={} status={}", addonXmlPath, static_cast<int>(loadStatus));
          continue;
        }

        info->plugin_path = static_cast<char*>(malloc(addonPath.length() + 1));
        if (info->plugin_path)
        {
          strncpy(info->plugin_path, addonPath.c_str(), addonPath.length());
          info->plugin_path[addonPath.length()] = '\0';
        }

        cp_status_t installStatus = cp_install_plugin(m_cp_context, info);
        CLog::Log(LOGNOTICE, "ADDONS: manual install path={} id={} status={}",
                  addonPath, info->identifier ? info->identifier : "<null>", static_cast<int>(installStatus));
      ...
      installed.clear();
      n = collectInstalled();
      CLog::Log(LOGNOTICE, "ADDONS: manual install fallback complete, count={}", n);
    }
#endif
    ...
    else
    {
      ...
      CLog::Log(LOGWARNING, "ADDONS: database unavailable, using scan-only addon state");
    }
#endif
  }
