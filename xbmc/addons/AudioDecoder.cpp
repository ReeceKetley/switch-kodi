...
bool CAudioDecoder::Init(const CFileItem& file, unsigned int filecache)
{
  ...
  // for replaygain
  std::unique_ptr<MUSIC_INFO::IMusicInfoTagLoader> tagLoader(
      MUSIC_INFO::CMusicInfoTagLoaderFactory::CreateLoader(file));
  if (tagLoader)
    tagLoader->Load(file.GetDynPath(), XFILE::CMusicFileDirectory::m_tag, nullptr);
