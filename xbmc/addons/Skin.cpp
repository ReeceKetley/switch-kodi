...
std::string ResolveSwitchSkinBasePath(const std::string& path, const std::string& addonId)
{
  if (StringUtils::EndsWithNoCase(path, ".zip"))
  {
    const std::string unpackedPath = URIUtils::AddFileToFolder("special://home/addons", addonId);
    if (CDirectory::Exists(unpackedPath))
      return unpackedPath;
  }
  return path;
}
...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  CLog::Log(LOGNOTICE, "SKIN: Start id=%s path=%s initial_res_count=%u", ID().c_str(), Path().c_str(), static_cast<unsigned>(m_resolutions.size()));
#endif
  if (!LoadUserSettings())
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  CLog::Log(LOGNOTICE, "SKIN: Start id=%s final_res_count=%u current_aspect=%s", ID().c_str(), static_cast<unsigned>(m_resolutions.size()), m_currentAspect.c_str());
#endif
}
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  strPathToUse = ResolveSwitchSkinBasePath(strPathToUse, ID());
#endif
...
{
  const std::string skinPath = GetSkinPath(strFile);
  const bool exists = CFile::Exists(skinPath);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  CLog::Log(LOGNOTICE, "SKIN: HasSkinFile id=%s file=%s path=%s exists=%d", ID().c_str(), strFile.c_str(), skinPath.c_str(), exists ? 1 : 0);
#endif
  return exists;
}
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  basePath = ResolveSwitchSkinBasePath(basePath, ID());
#endif

  RESOLUTION_INFO res;
  GetSkinPath("Home.xml", &res);
  if (!res.strMode.empty())
    paths.push_back(URIUtils::AddFileToFolder(basePath, res.strMode));
  if (res.strMode != m_defaultRes.strMode)
    paths.push_back(URIUtils::AddFileToFolder(basePath, m_defaultRes.strMode));
}
