...
namespace
{
bool SwitchListLayoutCfgContainsInt(const char* key, int value)
{
  if (!key || !*key)
    return false;
...
namespace
{
inline bool SwitchListLayoutCfgContainsInt(const char* key, int value) { return false; }
}
#endif
...
void CGUIListItemLayout::Render(CGUIListItem *item, int parentID)
{
  ...
  if (traceCall)
  {
    CLog::Log(LOGNOTICE, "SWITCH_LISTLAYOUT: render begin parent={} call={} item=0x{:x} skipGroup={}",
              parentID, static_cast<unsigned long long>(s_switchListLayoutRenderCalls),
  ...
  if (!skipGroupRender)
    m_group.DoRender();
  if (traceCall)
    CLog::Log(LOGNOTICE, "SWITCH_LISTLAYOUT: render done parent={}", parentID);
#else
  m_group.DoRender();
#endif
}
