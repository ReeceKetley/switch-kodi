...
static bool SwitchTextureCfgRead(char* outBuf, size_t outSize)
{
  if (!outBuf || outSize < 2)
    return false;
  ...
  for (const char* p : paths)
  {
    ...
    if (!f)
      continue;
    const size_t n = fread(outBuf, 1, outSize - 1, f);
    fclose(f);
    outBuf[n] = '\0';
    return true;
  }
...
static bool SwitchTextureCfgContainsInt(const char* key, int value)
{
  if (!key || !*key)
    return false;

  char buf[8192];
  if (!SwitchTextureCfgRead(buf, sizeof(buf)))
    return false;
...
static bool SwitchTextureCfgLineValueContains(const char* key, const std::string& text)
{
...
#else
static inline bool SwitchTextureCfgContainsInt(const char* key, int value) { return false; }
static inline bool SwitchTextureCfgLineValueContains(const char* key, const std::string& text) { return false; }
#endif
...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  const bool traceRender = SwitchTextureCfgContainsInt("trace_texture_render_all", 1) ||
                           SwitchTextureCfgLineValueContains("trace_texture_render_contains", m_info.filename);
  if (traceRender)
  {
    CLog::Log(LOGNOTICE,
              "SWITCH_TEX_RENDER: begin file='{}' visible={} tex_frames={} allocated={} failed={} pos=({},{}) size={}x{} frame={}x{}",
              m_info.filename, m_visible ? 1 : 0, static_cast<int>(m_texture.size()), IsAllocated() ? 1 : 0,
              FailedToAlloc() ? 1 : 0, m_posX, m_posY, m_width, m_height, m_frameWidth, m_frameHeight);
  }
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRender)
      CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER: early-return file='{}' reason={}{}",
                m_info.filename, !m_visible ? "invisible" : "", !m_texture.size() ? " no_texture" : "");
#endif
    return;
  }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRender)
      CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER: clip begin file='{}' pos=({},{}) size={}x{}",
                m_info.filename, m_posX, m_posY, m_width, m_height);
#endif
    if (!CServiceBroker::GetWinSystem()->GetGfxContext().SetClipRegion(m_posX, m_posY, m_width, m_height))
    {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceRender)
        CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER: clip reject file='{}'", m_info.filename);
#endif
      return;
    }
  }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER: pre-mergealpha file='{}' color=0x{:08x}", m_info.filename, color);
#endif
  color = CServiceBroker::GetWinSystem()->GetGfxContext().MergeAlpha(color);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER: post-mergealpha file='{}' color=0x{:08x}", m_info.filename, color);
#endif

  // setup our renderer
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER: pre-Begin file='{}'", m_info.filename);
#endif
  Begin(color);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER: post-Begin file='{}'", m_info.filename);
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER: texcoords ready file='{}' texPixels={} border=({},{}|{},{} )",
              m_info.filename, m_texture.m_texCoordsArePixels ? 1 : 0,
              m_info.border.x1, m_info.border.y1, m_info.border.x2, m_info.border.y2);
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER: done file='{}'", m_info.filename);
#endif
...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  const bool traceRenderStep = SwitchTextureCfgContainsInt("trace_texture_render_all", 1) ||
                               SwitchTextureCfgLineValueContains("trace_texture_render_contains", m_info.filename);
  if (traceRenderStep)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER2: seg-begin file='{}' ltrb=({},{},{},{}) uv=({},{},{},{})",
              m_info.filename, left, top, right, bottom, u1, v1, u2, v2);
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRenderStep)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER2: pre-cliprect file='{}'", m_info.filename);
#endif
  CServiceBroker::GetWinSystem()->GetGfxContext().ClipRect(vertex, texture, m_diffuse.size() ? &diffuse : NULL);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRenderStep)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER2: post-cliprect file='{}' v=({},{},{},{}) t=({},{},{},{})",
              m_info.filename, vertex.x1, vertex.y1, vertex.x2, vertex.y2,
              texture.x1, texture.y1, texture.x2, texture.y2);
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRenderStep)
      CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER2: seg-empty file='{}'", m_info.filename);
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRenderStep)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER2: pre-draw file='{}'", m_info.filename);
#endif
  Draw(x, y, z, texture, diffuse, orientation);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRenderStep)
    CLog::Log(LOGNOTICE, "SWITCH_TEX_RENDER2: post-draw file='{}'", m_info.filename);
#endif
}
...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  const bool traceTexture = SwitchTextureCfgContainsInt("trace_texture_alloc_all", 1) ||
                            SwitchTextureCfgLineValueContains("trace_texture_alloc_contains", m_info.filename) ||
                            StringUtils::EqualsNoCase(m_info.filename, "buttons/dialogbutton-fo.png") ||
                            StringUtils::EqualsNoCase(m_info.filename, "buttons/dialogbutton-nofo.png");
  if (traceTexture)
    CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc begin file='{}' visible={} allocated={} useLargeHint={}",
              m_info.filename, m_visible ? 1 : 0, IsAllocated() ? 1 : 0, m_info.useLarge ? 1 : 0);
#endif
  if (m_info.filename.empty())
  {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceTexture)
      CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc early-empty");
#endif
    return false;
  }
  if (m_texture.size())
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceTexture)
      CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc early-existing file='{}' frames={}", m_info.filename,
                static_cast<int>(m_texture.size()));
#endif
  ...
  bool canLoad = false;
  if (traceTexture)
    CLog::Log(LOGNOTICE, "SWITCH_TEX: canload begin file='{}'", m_info.filename);
  canLoad = CServiceBroker::GetGUI()->GetTextureManager().CanLoad(m_info.filename);
  if (traceTexture)
    CLog::Log(LOGNOTICE, "SWITCH_TEX: canload done file='{}' canLoad={}", m_info.filename, canLoad ? 1 : 0);
  bool useLarge = m_info.useLarge || !canLoad;
#else
  bool useLarge = m_info.useLarge || !CServiceBroker::GetGUI()->GetTextureManager().CanLoad(m_info.filename);
#endif
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceTexture)
    CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc loader_select file='{}' useLarge={}", m_info.filename, useLarge ? 1 : 0);
#endif
  if (useLarge)
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
        if (traceTexture)
          CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc texturemanager-hit file='{}' frames={}", m_info.filename,
                    static_cast<int>(m_texture.size()));
#endif
      }
    }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
          if (traceTexture)
            CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc large-pending file='{}'", m_info.filename);
#endif
          return false;
        }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
        if (traceTexture)
          CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc large-ready file='{}' frames={}", m_info.filename,
                    static_cast<int>(m_texture.size()));
#endif
      }
      else
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
        if (traceTexture)
          CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc large-failed file='{}'", m_info.filename);
#endif
      }
    }
  }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceTexture)
        CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc normal-failed file='{}'", m_info.filename);
#endif
      return false;
    }
    m_texture = texture;
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceTexture)
      CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc normal-ready file='{}' frames={}", m_info.filename,
                static_cast<int>(m_texture.size()));
#endif
  }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceTexture)
    CLog::Log(LOGNOTICE, "SWITCH_TEX: alloc done file='{}' changed={} frame={}x{}",
              m_info.filename, changed ? 1 : 0, m_frameWidth, m_frameHeight);
#endif
...
bool CGUITextureBase::CalculateSize()
{
  ...
  if (m_aspect.ratio != CAspectRatio::AR_STRETCH && m_frameWidth && m_frameHeight)
  {
    ...
    if (!std::isfinite(fOutputFrameRatio) || fOutputFrameRatio == 0.0f)
      fOutputFrameRatio = 1.0f;
  ...
  if (!std::isfinite(newPosX) || !std::isfinite(newPosY) || !std::isfinite(newWidth) || !std::isfinite(newHeight))
  {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    CLog::Log(LOGWARNING,
              "SWITCH_TEX: non-finite geometry corrected file='{}' pos=({},{}) size={}x{} calc=({},{},{},{})",
              m_info.filename, m_posX, m_posY, m_width, m_height, newPosX, newPosY, newWidth, newHeight);
#endif
    newPosX = m_posX;
    newPosY = m_posY;
    newWidth = (m_width > 0.0f && std::isfinite(m_width)) ? m_width : m_frameWidth;
    newHeight = (m_height > 0.0f && std::isfinite(m_height)) ? m_height : m_frameHeight;
  }
