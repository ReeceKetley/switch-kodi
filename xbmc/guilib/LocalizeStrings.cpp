    ...
    std::map<uint32_t, LocStr>& strings,  std::string &encoding, uint32_t offset = 0 )
{
  ...
  bool usedResourcesSubdir = false;
  bool allowSwitchResourcesRetry = false;
  if (!XFILE::CDirectory::Exists(pathname))
  {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    const std::string switchResourcesPath =
        CSpecialProtocol::TranslatePathConvertCase(pathname_in + language + "/resources");
    if (XFILE::CDirectory::Exists(switchResourcesPath))
    {
      pathname = switchResourcesPath;
      usedResourcesSubdir = true;
    }
    else
#endif
    {
  ...
  {
    allowSwitchResourcesRetry = true;
  }
#endif
  ...
  if (usedResourcesSubdir)
    useSourceLang = StringUtils::EqualsNoCase(language, LANGUAGE_DEFAULT);

  if (LoadPO(URIUtils::AddFileToFolder(pathname, "strings.po"), strings, encoding, offset, useSourceLang))
    return true;
  if (LoadXML(URIUtils::AddFileToFolder(pathname, "strings.xml"), strings, encoding, offset))
    return true;

#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (allowSwitchResourcesRetry && !usedResourcesSubdir)
  {
    const std::string switchResourcesPath =
        CSpecialProtocol::TranslatePathConvertCase(pathname_in + language + "/resources");
    if (XFILE::CDirectory::Exists(switchResourcesPath))
    {
      if (LoadPO(URIUtils::AddFileToFolder(switchResourcesPath, "strings.po"), strings, encoding, offset, useSourceLang))
        return true;
      if (LoadXML(URIUtils::AddFileToFolder(switchResourcesPath, "strings.xml"), strings, encoding, offset))
        return true;
    }
  }
#endif
