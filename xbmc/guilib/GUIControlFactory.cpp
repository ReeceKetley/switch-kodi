...
CGUIControl* CGUIControlFactory::Create(int parentID, const CRect &rect, TiXmlElement* pControlNode, bool insideContainer)
{
  ...
  case CGUIControl::GUICONTAINER_FIXEDLIST:
    {
      if (parentID == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_FACTORY: fixedlist begin id={}", id);
      CScroller scroller;
      GetScroller(pControlNode, "scrolltime", scroller);
      if (parentID == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_FACTORY: fixedlist scroller ok id={}", id);
      ...
      if (parentID == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_FACTORY: fixedlist ctor ok id={}", id);
      CGUIFixedListContainer* fcontrol = static_cast<CGUIFixedListContainer*>(control);
      if (parentID == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_FACTORY: fixedlist LoadLayout begin id={}", id);
      fcontrol->LoadLayout(pControlNode);
      if (parentID == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_FACTORY: fixedlist LoadLayout done id={}", id);
      if (parentID == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_FACTORY: fixedlist LoadListProvider begin id={}", id);
      fcontrol->LoadListProvider(pControlNode, defaultControl, defaultAlways);
      if (parentID == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_FACTORY: fixedlist LoadListProvider done id={}", id);
      ...
      if (parentID == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_FACTORY: fixedlist done id={}", id);
    }
    break;
