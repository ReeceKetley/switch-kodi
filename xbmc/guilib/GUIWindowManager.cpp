...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
static void SwitchWmTrace(const char* fmt, ...)
{
  const char* paths[] = {
...
static bool SwitchWmCfgHas(const char* key)
{
  if (!key)
    return false;
...
#else
static inline void SwitchWmTrace(const char* fmt, ...) {}
static inline bool SwitchWmCfgHas(const char* key) { return false; }
#endif
...
{
  SwitchWmTrace("SWITCH_WM: Initialize begin");
  m_tracker.SelectAlgorithm();
  SwitchWmTrace("SWITCH_WM: SelectAlgorithm done");

  m_initialized = true;

#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  SwitchWmTrace("SWITCH_WM: LoadNotOnDemandWindows skipped on Switch");
#else
  LoadNotOnDemandWindows();
  SwitchWmTrace("SWITCH_WM: LoadNotOnDemandWindows done");
#endif

  CApplicationMessenger::GetInstance().RegisterReceiver(this);
  SwitchWmTrace("SWITCH_WM: RegisterReceiver done");
}
...
{
  SwitchWmTrace("SWITCH_WM: ActivateWindow request id=%d swap=%d force=%d", iWindowID, swappingWindows ? 1 : 0, force ? 1 : 0);
...
{
  SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal enter id=%d", iWindowID);
  ...
  { // nothing to see here - move along
    SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal missing window id=%d", iWindowID);
  ...
  {
    SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal cannot activate id=%d", iWindowID);
    if (IsWindowActive(WINDOW_STARTUP_ANIM))
      ActivateWindow(WINDOW_HOME);
    return;
  }
  ...
  { // if we have a dialog, we do a DoModal() rather than activate the window
    SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal dialog path id=%d", iWindowID);
  ...
  {
    SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal blocked by modal dialog id=%d", iWindowID);
  ...
  // deactivate any window
  int currentWindow = GetActiveWindow();
  SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal current=%d next=%d", currentWindow, iWindowID);
  CGUIWindow *pWindow = GetWindow(currentWindow);
  if (pWindow)
  {
    SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal CloseWindowSync begin current=%d", currentWindow);
    CloseWindowSync(pWindow, iWindowID);
    SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal CloseWindowSync done current=%d", currentWindow);
  }
  CServiceBroker::GetGUI()->GetInfoManager().GetInfoProviders().GetGUIControlsInfoProvider().SetNextWindow(WINDOW_INVALID);
  ...
  SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal WINDOW_INIT begin id=%d", iWindowID);
  pNewWindow->OnMessage(msg);
  SwitchWmTrace("SWITCH_WM: ActivateWindow_Internal WINDOW_INIT done id=%d", iWindowID);
//  CServiceBroker::GetGUI()->GetInfoManager().GetInfoProviders().GetGUIControlsInfoProvider().SetPreviousWindow(WINDOW_INVALID);
}
...
void CGUIWindowManager::Process(unsigned int currentTime)
{
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  static uint64_t s_switchWmProcessCalls = 0;
  ++s_switchWmProcessCalls;
  const bool wmTrace = (s_switchWmProcessCalls <= 10);
  if (wmTrace)
    CLog::Log(LOGNOTICE, "SWITCH_WMP: enter call={}", static_cast<unsigned long long>(s_switchWmProcessCalls));
#endif
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (wmTrace)
    CLog::Log(LOGNOTICE, "SWITCH_WMP: gfx lock begin");
#endif
  CSingleLock lock(CServiceBroker::GetWinSystem()->GetGfxContext());
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (wmTrace)
    CLog::Log(LOGNOTICE, "SWITCH_WMP: gfx lock done");
#endif

  m_dirtyregions.clear();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (wmTrace)
    CLog::Log(LOGNOTICE, "SWITCH_WMP: dirty clear done");
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (wmTrace)
    CLog::Log(LOGNOTICE, "SWITCH_WMP: active id={}", activeWindowId);
#endif
  auto activeIt = m_mapWindows.find(activeWindowId);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (wmTrace)
    CLog::Log(LOGNOTICE, "SWITCH_WMP: active find done found={}", activeIt != m_mapWindows.end() ? 1 : 0);
#endif
  if (activeIt != m_mapWindows.end())
    pWindow = activeIt->second;
  if (pWindow)
  {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    const bool switchBypassActiveDoProcess = false; // Keep real behavior; trace only.
    if (wmTrace)
      CLog::Log(LOGNOTICE, "SWITCH_WMP: active DoProcess begin ptr=0x{:x}",
                static_cast<unsigned long long>(reinterpret_cast<uintptr_t>(pWindow)));
    if (switchBypassActiveDoProcess)
    {
      if (wmTrace)
        CLog::Log(LOGNOTICE, "SWITCH_WMP: active DoProcess bypassed");
    }
    else
    {
      pWindow->DoProcess(currentTime, m_dirtyregions);
      if (wmTrace)
        CLog::Log(LOGNOTICE, "SWITCH_WMP: active DoProcess done ptr=0x{:x}",
                  static_cast<unsigned long long>(reinterpret_cast<uintptr_t>(pWindow)));
    }
#else
    pWindow->DoProcess(currentTime, m_dirtyregions);
  ...
  for (const auto& entry : m_mapWindows)
  {
    ...
    {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      const int dialogId = pWindow->GetID();
      const bool switchBypassDialogDoProcess = false;
      if (wmTrace)
        CLog::Log(LOGNOTICE, "SWITCH_WMP: dialog DoProcess begin id={}", dialogId);
      if (switchBypassDialogDoProcess)
      {
        if (wmTrace)
          CLog::Log(LOGNOTICE, "SWITCH_WMP: dialog DoProcess bypassed id={}", dialogId);
      }
      else
      {
        pWindow->DoProcess(currentTime, m_dirtyregions);
        if (wmTrace)
          CLog::Log(LOGNOTICE, "SWITCH_WMP: dialog DoProcess done id={}", dialogId);
      }
#else
      pWindow->DoProcess(currentTime, m_dirtyregions);
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (wmTrace)
    CLog::Log(LOGNOTICE, "SWITCH_WMP: done call={}", static_cast<unsigned long long>(s_switchWmProcessCalls));
#endif
}
...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  const bool traceRender = SwitchWmCfgHas("trace_render_steps=1");
#endif
  CGUIWindow* pWindow = GetWindow(GetActiveWindow());
  if (pWindow)
  {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRender)
      CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: active ClearBackground begin id={}", pWindow->GetID());
#endif
    pWindow->ClearBackground();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRender)
      CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: active ClearBackground done id={}", pWindow->GetID());
    if (traceRender)
      CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: active DoRender begin id={}", pWindow->GetID());
#endif
    pWindow->DoRender();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRender)
      CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: active DoRender done id={}", pWindow->GetID());
#endif
  }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceRender)
        CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: dialog DoRender begin id={}", window->GetID());
#endif
      window->DoRender();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceRender)
        CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: dialog DoRender done id={}", window->GetID());
#endif
    }
  }
}
...
bool CGUIWindowManager::Render()
{
  ...
  const bool traceRender = SwitchWmCfgHas("trace_render_steps=1");
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: Render enter");
#endif
  CSingleExit lock(CServiceBroker::GetWinSystem()->GetGfxContext());
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: Render after CSingleExit");
#endif

  CDirtyRegionList dirtyRegions = m_tracker.GetDirtyRegions();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: Render dirty regions={}",
              static_cast<int>(dirtyRegions.size()));
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRender)
      CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: Render full viewport (always)");
#endif
    RenderPass();
    hasRendered = true;
  }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceRender)
        CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: Render full viewport (on change)");
#endif
      RenderPass();
      hasRendered = true;
    }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRender)
      CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: Render dirty rect loop begin");
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRender)
      CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: Render dirty rect loop done");
#endif
  }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRender)
    CLog::Log(LOGNOTICE, "SWITCH_WM_RENDER: Render return hasRendered={}", hasRendered ? 1 : 0);
#endif
  return hasRendered;
}
...
{
  SwitchWmTrace("SWITCH_WM: LoadNotOnDemandWindows begin");
  CSingleLock lock(CServiceBroker::GetWinSystem()->GetGfxContext());
  for (const auto& entry : m_mapWindows)
  {
    CGUIWindow *pWindow = entry.second;
    ...
    {
      SwitchWmTrace("SWITCH_WM: init window begin id=%d", pWindow->GetID());
      pWindow->FreeResources(true);
      pWindow->Initialize();
      SwitchWmTrace("SWITCH_WM: init window done id=%d", pWindow->GetID());
    }
  }
  SwitchWmTrace("SWITCH_WM: LoadNotOnDemandWindows end");
}
