...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  const bool traceGles = m_info.filename == "icons/logo.png" ||
                         m_info.filename == "dialogs/close.png" ||
                         m_info.filename == "icons/power.png";
  if (traceGles)
    CLog::Log(LOGNOTICE, "SWITCH_GLES: Begin enter file='{}' frame={} textures={} renderSystem=0x{:x}",
              m_info.filename, m_currentFrame, static_cast<int>(m_texture.size()),
              static_cast<unsigned long long>(reinterpret_cast<uintptr_t>(m_renderSystem)));
#endif
  if (m_currentFrame >= m_texture.m_textures.size())
  {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceGles)
      CLog::Log(LOGERROR, "SWITCH_GLES: Begin invalid frame index file='{}' frame={} textures={}",
                m_info.filename, m_currentFrame, static_cast<int>(m_texture.m_textures.size()));
#endif
    return;
  }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceGles)
      CLog::Log(LOGERROR, "SWITCH_GLES: Begin null texture ptr file='{}' frame={}", m_info.filename, m_currentFrame);
#endif
    return;
  }
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceGles)
    CLog::Log(LOGNOTICE, "SWITCH_GLES: Begin pre-LoadToGPU file='{}'", m_info.filename);
#endif
  texture->LoadToGPU();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceGles)
    CLog::Log(LOGNOTICE, "SWITCH_GLES: Begin post-LoadToGPU file='{}'", m_info.filename);
#endif
  if (m_diffuse.size())
  {
    if (m_diffuse.m_textures.empty() || m_diffuse.m_textures[0] == nullptr)
    {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceGles)
        CLog::Log(LOGERROR, "SWITCH_GLES: Begin diffuse invalid file='{}' diffuse_size={}",
                  m_info.filename, static_cast<int>(m_diffuse.m_textures.size()));
#endif
    }
    else
    {
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceGles)
    CLog::Log(LOGNOTICE, "SWITCH_GLES: Begin pre-BindToUnit file='{}'", m_info.filename);
#endif
  texture->BindToUnit(0);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceGles)
    CLog::Log(LOGNOTICE, "SWITCH_GLES: Begin post-BindToUnit file='{}'", m_info.filename);
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceGles)
    CLog::Log(LOGNOTICE, "SWITCH_GLES: Begin done file='{}' hasAlpha={}", m_info.filename, hasAlpha ? 1 : 0);
#endif
...
void CGUITextureGLES::End()
{
  ...
  const bool traceGles = m_info.filename == "icons/logo.png" ||
                         ...
                         m_info.filename == "icons/power.png";
  if (traceGles)
    CLog::Log(LOGNOTICE, "SWITCH_GLES: End enter file='{}' packed_verts={}",
              m_info.filename, static_cast<int>(m_packedVertices.size()));
  if (traceGles && !m_packedVertices.empty())
  {
    float minX = m_packedVertices[0].x;
    float maxX = m_packedVertices[0].x;
    float minY = m_packedVertices[0].y;
    float maxY = m_packedVertices[0].y;
    for (const auto& v : m_packedVertices)
    {
    ...
    CLog::Log(LOGNOTICE,
              "SWITCH_GLES: End quad file='{}' bounds=({:.2f},{:.2f})-({:.2f},{:.2f}) color=({}, {}, {}, {})",
              m_info.filename, minX, minY, maxX, maxY,
              static_cast<int>(m_col[0]), static_cast<int>(m_col[1]),
              static_cast<int>(m_col[2]), static_cast<int>(m_col[3]));
  }
#endif
  if (m_packedVertices.size())
  {
    ...
    const GLsizei drawCount = static_cast<GLsizei>(m_packedVertices.size() * 6 / 4);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceGles)
      CLog::Log(LOGNOTICE, "SWITCH_GLES: End pre-glDrawElements file='{}' drawCount={} idx_size={}",
                m_info.filename, drawCount, static_cast<int>(m_idx.size()));
#endif
    glDrawElements(GL_TRIANGLES, drawCount, GL_UNSIGNED_SHORT, m_idx.data());
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceGles)
      CLog::Log(LOGNOTICE, "SWITCH_GLES: End post-glDrawElements file='{}'", m_info.filename);
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceGles)
    CLog::Log(LOGNOTICE, "SWITCH_GLES: End done file='{}'", m_info.filename);
#endif
}
