...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
static void SwitchGuiWindowTrace(const char* fmt, ...)
{
  const char* paths[] = {
...
static bool SwitchGuiWindowCfgHas(const char* key)
{
  if (!key)
    return false;
...
static int SwitchGuiWindowCfgGetInt(const char* key, int defaultValue)
{
  if (!key || !*key)
    return defaultValue;
...
#else
static inline void SwitchGuiWindowTrace(const char* fmt, ...) {}
static inline bool SwitchGuiWindowCfgHas(const char* key) { return false; }
static inline int SwitchGuiWindowCfgGetInt(const char* key, int defaultValue) { return defaultValue; }
#endif
...
{
  const bool isHome = (GetID() == WINDOW_HOME);
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load enter id={} file={} hasPath={}", GetID(), strFileName.c_str(), bContainsPath ? 1 : 0);
  if (m_windowLoaded || !g_SkinInfo)
  {
    if (isHome)
      CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load early return id={} loaded={} skin={}", GetID(), m_windowLoaded ? 1 : 0, g_SkinInfo ? 1 : 0);
  ...
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load after LoadXML id={} ret={}", GetID(), ret ? 1 : 0);
  if (ret)
  {
    m_windowLoaded = true;
    OnWindowLoaded();
  ...
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load done id={} ret={}", GetID(), ret ? 1 : 0);
  return ret;
}
...
bool CGUIWindow::LoadXML(const std::string &strPath, const std::string &strLowerPath)
{
  const bool isHome = (GetID() == WINDOW_HOME);
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: LoadXML enter id={} path={}", GetID(), strPath.c_str());
  ...
  if (!m_windowXMLRootElement)
  {
    ...
    if (isHome)
      CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: LoadXML cloned root id={}", GetID());
  }
  else
    CLog::Log(LOGDEBUG, "Using already stored xml root node for %s", strPath.c_str());

  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: LoadXML Prepare begin id={}", GetID());
  auto prepared = Prepare(m_windowXMLRootElement);
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: LoadXML Prepare done id={} ok={}", GetID(), prepared ? 1 : 0);
  const bool loaded = Load(prepared.get());
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: LoadXML Load(TiXml) done id={} ret={}", GetID(), loaded ? 1 : 0);
  return loaded;
}
...
std::unique_ptr<TiXmlElement> CGUIWindow::Prepare(TiXmlElement *pRootElement)
{
  const bool isHome = (GetID() == WINDOW_HOME);
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Prepare enter id={} root={}", GetID(), pRootElement ? 1 : 0);
  if (!pRootElement)
    return nullptr;
  ...
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Prepare cloned root id={}", GetID());

  // Resolve any includes, constants, expressions that may be present
  // and save include's conditions to the given map
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Prepare ResolveIncludes begin id={}", GetID());
  g_SkinInfo->ResolveIncludes(preparedRoot.get(), &m_xmlIncludeConditions);
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Prepare ResolveIncludes done id={}", GetID());
...
bool CGUIWindow::Load(TiXmlElement *pRootElement)
{
  const bool isHome = (GetID() == WINDOW_HOME);
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) enter id={} root={}", GetID(), pRootElement ? 1 : 0);
  if (!pRootElement)
    return false;
  ...
  SetDefaults();
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) SetDefaults done id={}", GetID());
  ...
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) child loop begin id={}", GetID());
  int childIndex = 0;
  while (pChild)
  {
    std::string strValue = pChild->Value();
    if (isHome)
      CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) child begin idx={} name={}", childIndex, strValue.c_str());
    ...
    else if (strValue == "controls")
    {
      ...
      int controlIndex = 0;
      const int skipHomeControlIdx = (isHome ? SwitchGuiWindowCfgGetInt("skip_home_control_idx", -1) : -1);
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) controls begin id={}", GetID());
      while (pControl)
      {
        if (StringUtils::EqualsNoCase(pControl->Value(), "control"))
        {
          const char* controlType = pControl->Attribute("type");
          const char* controlId = pControl->Attribute("id");
          if (isHome)
            CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) control begin idx={} type={} id={}",
                      controlIndex,
                      controlType ? controlType : "",
                      controlId ? controlId : "");
          if (isHome && controlIndex == skipHomeControlIdx)
          {
            CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) control skipped idx={} type={} id={}",
                      controlIndex,
                      controlType ? controlType : "",
                      controlId ? controlId : "");
          }
          ...
          if (isHome)
            CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) control done idx={} type={} id={}",
                      controlIndex,
                      controlType ? controlType : "",
                      controlId ? controlId : "");
        }
      ...
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) controls done id={}", GetID());
    }

    pChild = pChild->NextSiblingElement();
    if (isHome)
      CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) child done idx={} name={}", childIndex, strValue.c_str());
    ++childIndex;
  }

  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: Load(TiXml) child loop done id={}", GetID());
  return true;
}
...
{
  const bool isHome = (GetID() == WINDOW_HOME);
  static thread_local int s_homeLoadDepth = 0;
  const char* controlType = pControl ? pControl->Attribute("type") : nullptr;
  const char* controlId = pControl ? pControl->Attribute("id") : nullptr;
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: LoadControl begin depth={} type={} id={} group={}",
              s_homeLoadDepth,
              controlType ? controlType : "",
              controlId ? controlId : "",
              pGroup ? 1 : 0);
  ...
  if (pGUIControl)
  {
    ...
    if (pGUIControl->IsGroup())
    {
      ...
      {
        if (isHome)
          CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: LoadControl subgroup begin parentType={} parentId={} subIdx={}",
                    controlType ? controlType : "",
                    controlId ? controlId : "",
                    subIndex);
        LoadControl(pSubControl, grp, grpRect);
        if (isHome)
          CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: LoadControl subgroup done parentType={} parentId={} subIdx={}",
                    controlType ? controlType : "",
                    controlId ? controlId : "",
                    subIndex);
  ...
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: LoadControl done depth={} type={} id={} created={}",
              s_homeLoadDepth,
              controlType ? controlType : "",
              controlId ? controlId : "",
              pGUIControl ? 1 : 0);
...
bool CGUIWindow::OnMessage(CGUIMessage& message)
{
  ...
  case GUI_MSG_WINDOW_INIT:
    {
      SwitchGuiWindowTrace("SWITCH_GUIWIN: WINDOW_INIT begin id=%d xml=%s", GetID(), GetProperty("xmlfile").c_str());
      if (GetID() == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: WINDOW_INIT begin id={} xml={}", GetID(), GetProperty("xmlfile").c_str());
      CLog::Log(LOGDEBUG, "------ Window Init (%s) ------", GetProperty("xmlfile").c_str());
      if (m_dynamicResourceAlloc || !m_bAllocated)
      {
        if (GetID() == WINDOW_HOME && SwitchGuiWindowCfgHas("skip_home_alloc=1"))
        {
          SwitchGuiWindowTrace("SWITCH_GUIWIN: WINDOW_INIT alloc skipped id=%d", GetID());
          CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: WINDOW_INIT alloc skipped id={}", GetID());
        }
        else
        {
          SwitchGuiWindowTrace("SWITCH_GUIWIN: WINDOW_INIT AllocResources begin id=%d", GetID());
          if (GetID() == WINDOW_HOME)
            CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: WINDOW_INIT AllocResources begin id={}", GetID());
          AllocResources(false);
          SwitchGuiWindowTrace("SWITCH_GUIWIN: WINDOW_INIT AllocResources done id=%d", GetID());
          if (GetID() == WINDOW_HOME)
            CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: WINDOW_INIT AllocResources done id={}", GetID());
        }
      }
      SwitchGuiWindowTrace("SWITCH_GUIWIN: WINDOW_INIT OnInitWindow begin id=%d", GetID());
      if (GetID() == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: WINDOW_INIT OnInitWindow begin id={}", GetID());
      OnInitWindow();
      SwitchGuiWindowTrace("SWITCH_GUIWIN: WINDOW_INIT OnInitWindow done id=%d", GetID());
      if (GetID() == WINDOW_HOME)
        CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: WINDOW_INIT OnInitWindow done id={}", GetID());
      return true;
    }
    break;
...
void CGUIWindow::AllocResources(bool forceLoad /*= false */)
{
  ...
  const bool isHome = (GetID() == WINDOW_HOME);
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: AllocResources enter id={} forceLoad={}", GetID(), forceLoad ? 1 : 0);
  ...
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: AllocResources after NeedLoad id={} forceLoad={}", GetID(), forceLoad ? 1 : 0);

  // if window is loaded and load is forced we have to free window resources first
  if (m_windowLoaded && forceLoad)
  {
    if (isHome)
      CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: AllocResources FreeResources begin id={}", GetID());
    FreeResources(true);
    if (isHome)
      CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: AllocResources FreeResources done id={}", GetID());
  }
  ...
  if (forceLoad)
  {
    ...
    {
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: AllocResources Load begin id={} xml={}", GetID(), xmlFile.c_str());
      bool bHasPath = xmlFile.find("\\") != std::string::npos || xmlFile.find("/") != std::string::npos;
      Load(xmlFile, bHasPath);
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: AllocResources Load done id={}", GetID());
    }
  }
  ...
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: AllocResources GroupAlloc begin id={}", GetID());
  CGUIControlGroup::AllocResources();
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: AllocResources GroupAlloc done id={}", GetID());
  ...
  m_bAllocated = true;
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_GUIWIN: AllocResources done id={}", GetID());
}
