...
#include <cstdio>
#include <cstring>
static bool SwitchRadioCfgHas(const char* key)
{
  if (!key)
    return false;
  const char* paths[] = {
...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  const bool traceRadio = SwitchRadioCfgHas("trace_render_steps=1");
  if (traceRadio)
  {
    CLog::Log(LOGNOTICE, "SWITCH_RADIO: render begin parent={} id={} selected={} disabled={} focus={} onFo='{}' onNoFo='{}' offFo='{}' offNoFo='{}' onDis='{}' offDis='{}'",
              GetParentID(), GetID(), IsSelected() ? 1 : 0, IsDisabled() ? 1 : 0, HasFocus() ? 1 : 0,
              m_imgRadioOnFocus.GetFileName(), m_imgRadioOnNoFocus.GetFileName(),
              m_imgRadioOffFocus.GetFileName(), m_imgRadioOffNoFocus.GetFileName(),
              m_imgRadioOnDisabled.GetFileName(), m_imgRadioOffDisabled.GetFileName());
  }
#endif
  CGUIButtonControl::Render();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRadio)
    CLog::Log(LOGNOTICE, "SWITCH_RADIO: after button render parent={} id={}", GetParentID(), GetID());
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceRadio)
        CLog::Log(LOGNOTICE, "SWITCH_RADIO: draw m_imgRadioOnFocus parent={} id={}", GetParentID(), GetID());
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceRadio)
        CLog::Log(LOGNOTICE, "SWITCH_RADIO: draw m_imgRadioOnNoFocus parent={} id={}", GetParentID(), GetID());
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceRadio)
        CLog::Log(LOGNOTICE, "SWITCH_RADIO: draw m_imgRadioOffFocus parent={} id={}", GetParentID(), GetID());
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceRadio)
        CLog::Log(LOGNOTICE, "SWITCH_RADIO: draw m_imgRadioOffNoFocus parent={} id={}", GetParentID(), GetID());
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRadio)
      CLog::Log(LOGNOTICE, "SWITCH_RADIO: draw m_imgRadioOnDisabled parent={} id={}", GetParentID(), GetID());
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (traceRadio)
      CLog::Log(LOGNOTICE, "SWITCH_RADIO: draw m_imgRadioOffDisabled parent={} id={}", GetParentID(), GetID());
#endif
    m_imgRadioOffDisabled.Render();
  }
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceRadio)
    CLog::Log(LOGNOTICE, "SWITCH_RADIO: render done parent={} id={}", GetParentID(), GetID());
#endif
}
