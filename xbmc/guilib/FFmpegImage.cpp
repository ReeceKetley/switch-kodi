...
// and bufferSize (EOF sentinel)
static inline size_t Clamp(int64_t newPosition, size_t bufferSize)
{
  return std::min(std::max((int64_t) 0, newPosition), (int64_t)bufferSize);
}
...
static int64_t mem_file_seek(void *h, int64_t pos, int whence)
{
  ...
  {
    mbuf->pos = Clamp(((int64_t)mbuf->size) + pos, mbuf->size);
  }
  else
  {
...
static AVCodecID DetectImageCodec(const std::string& mime, const unsigned char* buffer, unsigned int bufSize)
{
  if (bufSize > 8 && buffer &&
  ...
  if (StringUtils::EqualsNoCase(mime, "image/png") || StringUtils::EqualsNoCase(mime, "image/apng"))
    return AV_CODEC_ID_PNG;
  if (StringUtils::EqualsNoCase(mime, "image/jpeg") || StringUtils::EqualsNoCase(mime, "image/jpg"))
    return AV_CODEC_ID_MJPEG;
  if (StringUtils::EqualsNoCase(mime, "image/webp"))
    return AV_CODEC_ID_WEBP;
  if (StringUtils::EqualsNoCase(mime, "image/gif"))
    return AV_CODEC_ID_GIF;
  if (StringUtils::EqualsNoCase(mime, "image/tiff"))
                                      ...
                                      unsigned int width, unsigned int height)
{
  ...
  // Switch-safe direct decode path that avoids avformat custom-IO hangs on tiny images.
  const AVCodecID directCodec = DetectImageCodec(m_strMimeType, buffer, bufSize);
  if (directCodec != AV_CODEC_ID_NONE && buffer && bufSize > 0)
  {
    av_frame_free(&m_pFrame);
    m_pFrame = nullptr;

    AVCodec* codec = avcodec_find_decoder(directCodec);
    if (codec)
    {
      AVCodecContext* directCtx = avcodec_alloc_context3(codec);
      if (directCtx && avcodec_open2(directCtx, codec, NULL) >= 0)
      {
        AVPacket* pkt = av_packet_alloc();
        AVFrame* frame = av_frame_alloc();
        if (pkt && frame && av_new_packet(pkt, static_cast<int>(bufSize)) >= 0)
        {
          memcpy(pkt->data, buffer, bufSize);
          if (avcodec_send_packet(directCtx, pkt) >= 0 && avcodec_receive_frame(directCtx, frame) >= 0)
          {
            m_pFrame = frame;
            ...
            av_packet_free(&pkt);
            avcodec_free_context(&directCtx);
            return true;
          }
        }
        if (frame)
