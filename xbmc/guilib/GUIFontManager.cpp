...
#if defined(TARGET_SWITCH)
struct SwitchFontTraceConfig
{
  bool loaded = false;
  bool traceFonts = false;
  std::string skipFontFile;
  int skipFontIndex = -1;
  int maxFontIndex = -1;
};

static SwitchFontTraceConfig& GetSwitchFontTraceConfig()
{
  static SwitchFontTraceConfig cfg;
  if (cfg.loaded)
    return cfg;
  ...
  for (const char* p : cfgPaths)
  {
    ...
    if (strstr(buf, "trace_fonts=1"))
      cfg.traceFonts = true;

    const char* key = "skip_font_file=";
    ...
    if (at)
    {
      ...
      while (*eol && *eol != '\n' && *eol != '\r')
        ++eol;
      cfg.skipFontFile.assign(at, static_cast<size_t>(eol - at));
      StringUtils::Trim(cfg.skipFontFile);
      StringUtils::ToLower(cfg.skipFontFile);
    }

    const char* keySkipIdx = "skip_font_index=";
    at = strstr(buf, keySkipIdx);
    if (at)
    {
      at += strlen(keySkipIdx);
      cfg.skipFontIndex = std::atoi(at);
    }

    const char* keyMaxIdx = "max_font_index=";
    at = strstr(buf, keyMaxIdx);
    if (at)
    {
      at += strlen(keyMaxIdx);
      cfg.maxFontIndex = std::atoi(at);
    }
    break;
  }
...
CGUIFont* GUIFontManager::LoadTTF(const std::string& strFontName, const std::string& strFilename, UTILS::Color textColor, UTILS::Color shadowColor, const int iSize, const int iStyle, bool border, float lineSpacing, float aspect, const RESOLUTION_INFO *sourceRes, bool preserveAspect)
{
...
#if defined(TARGET_SWITCH)
  if (!std::isfinite(newSize) || newSize <= 0.0f)
  {
    CLog::Log(LOGWARNING, "SWITCH_FONT: invalid scaled size={} for {} (base={}), forcing fallback",
              newSize, strFontName, iSize);
    newSize = static_cast<float>(iSize > 0 ? iSize : 20);
  }
  if (!std::isfinite(aspect) || aspect <= 0.0f)
  {
    CLog::Log(LOGWARNING, "SWITCH_FONT: invalid scaled aspect={} for {} (base={}), forcing 1.0",
              aspect, strFontName, originalAspect);
    aspect = 1.0f;
  }
#endif
...
#if defined(TARGET_SWITCH)
  const auto& switchCfg = GetSwitchFontTraceConfig();
  if (switchCfg.traceFonts)
  {
    CLog::Log(LOGNOTICE, "SWITCH_FONT: LoadTTF start name={} file={} size={} aspect={}",
              strFontName, strFilename, iSize, aspect);
  }
#endif
  ...
  if (switchCfg.traceFonts)
  {
    CLog::Log(LOGNOTICE, "SWITCH_FONT: resolved path name={} file={} path={} exists={}",
              strFontName, strFilename, strPath, XFILE::CFile::Exists(strPath) ? 1 : 0);
  }
#endif
  ...
  if (!pFontFile)
  {
    ...
    if (switchCfg.traceFonts)
    {
      CLog::Log(LOGNOTICE, "SWITCH_FONT: pFontFile->Load begin name={} ttf={} path={}",
                strFontName, TTFfontName, strPath);
    }
#endif
    ...
    if (switchCfg.traceFonts)
    {
      CLog::Log(LOGNOTICE, "SWITCH_FONT: pFontFile->Load done name={} loaded={}",
                strFontName, bFontLoaded ? 1 : 0);
    }
#endif
...
void GUIFontManager::LoadFonts(const TiXmlNode* fontNode)
{
  ...
  while (fontNode)
  {
    ...
    if (!fontName.empty() && URIUtils::HasExtension(fileName, ".ttf"))
    {
...
#if defined(TARGET_SWITCH)
      if (switchCfg.maxFontIndex >= 0 && fontIndex > switchCfg.maxFontIndex)
      {
        if (switchCfg.traceFonts)
        {
          CLog::Log(LOGNOTICE, "SWITCH_FONT: stop at index={} due to max_font_index={}",
                    fontIndex, switchCfg.maxFontIndex);
        }
        break;
      }
      if (switchCfg.skipFontIndex >= 0 && fontIndex == switchCfg.skipFontIndex)
      {
        CLog::Log(LOGWARNING, "SWITCH_FONT: skipping font index={} file={} name={}",
                  fontIndex, strFontFileName, fontName);
        fontNode = fontNode->NextSibling("font");
        ++fontIndex;
        continue;
      }
      if (!switchCfg.skipFontFile.empty() && StringUtils::EqualsNoCase(strFontFileName, switchCfg.skipFontFile))
      {
        CLog::Log(LOGWARNING, "SWITCH_FONT: skipping font file={} name={}", strFontFileName, fontName);
        fontNode = fontNode->NextSibling("font");
        ++fontIndex;
        continue;
      }
      if (switchCfg.traceFonts)
      {
        CLog::Log(LOGNOTICE, "SWITCH_FONT: node begin idx={} name={} file={}",
                  fontIndex, fontName, strFontFileName);
      }
#endif
      ...
      if (switchCfg.traceFonts)
      {
        CLog::Log(LOGNOTICE, "SWITCH_FONT: node done idx={} name={} file={}",
                  fontIndex, fontName, strFontFileName);
      }
      ++fontIndex;
#endif
