...
static bool SwitchTraceGlyphs()
{
  ...
  for (const char* p : paths)
  {
    ...
    fclose(f);
    buf[n] = '\0';
    if (std::strstr(buf, "trace_glyphs=1"))
      cached = 1;
    break;
  }
  return cached == 1;
...
static bool SwitchIsAsciiishByte(uint8_t b)
{
  return b == 0 || b == '\n' || b == '\r' || b == '\t' || (b >= 0x20 && b <= 0x7e);
}

static bool SwitchLooksPackedTextChar(character_t chr)
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  const uint8_t b0 = static_cast<uint8_t>(chr & 0xFF);
  const uint8_t b1 = static_cast<uint8_t>((chr >> 8) & 0xFF);
  const uint8_t b2 = static_cast<uint8_t>((chr >> 16) & 0xFF);
  const uint8_t b3 = static_cast<uint8_t>((chr >> 24) & 0xFF);
  return SwitchIsAsciiishByte(b0) && SwitchIsAsciiishByte(b1) &&
         SwitchIsAsciiishByte(b2) && SwitchIsAsciiishByte(b3) &&
         (b2 >= 0x20 || b3 >= 0x20);
#else
  (void)chr;
  return false;
...
static void SwitchAppendExpandedChar(character_t chr, vecText& out)
{
  const uint8_t b0 = static_cast<uint8_t>(chr & 0xFF);
  const uint8_t b1 = static_cast<uint8_t>((chr >> 8) & 0xFF);
  const uint8_t b2 = static_cast<uint8_t>((chr >> 16) & 0xFF);
  const uint8_t b3 = static_cast<uint8_t>((chr >> 24) & 0xFF);
  if (b0) out.push_back(static_cast<character_t>(b0));
  if (b1) out.push_back(static_cast<character_t>(b1));
  if (b2) out.push_back(static_cast<character_t>(b2));
  if (b3) out.push_back(static_cast<character_t>(b3));
}
...
bool CGUIFontTTFBase::Load(const std::string& strFilename, float height, float aspect, float lineSpacing, bool border)
{
  ...
  // Force Unicode cmap so ASCII/localized UI strings map to expected glyphs.
  if (FT_Select_Charmap(m_face, FT_ENCODING_UNICODE) != 0)
  {
    CLog::Log(LOGWARNING, "SWITCH_FONT: failed to select unicode charmap for {}", strFilename);
  }
...
void CGUIFontTTFBase::DrawTextInternal(float x, float y, const std::vector<UTILS::Color> &colors, const vecText &text, uint32_t alignment, float maxPixelWidth, bool scrolling)
{
  ...
  const vecText* textToRender = &text;
  vecText expandedText;
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  bool hasPacked = false;
  for (character_t ch : text)
  {
    if (SwitchLooksPackedTextChar(ch))
    {
      hasPacked = true;
      break;
    }
  ...
  if (hasPacked)
  {
    expandedText.reserve(text.size() * 4);
    for (character_t ch : text)
    {
      if (SwitchLooksPackedTextChar(ch))
        SwitchAppendExpandedChar(ch, expandedText);
      else
        expandedText.push_back(ch);
    }
    textToRender = &expandedText;
  }
#endif
  ...
  if (dirtyCache)
  {
    ...
    if ( alignment & (XBFONT_RIGHT | XBFONT_CENTER_X) )
    {
      ...
      float w = GetTextWidthInternal(textToRender->begin(), textToRender->end());
    ...
    if ( alignment & XBFONT_JUSTIFIED )
    {
      ...
      float linePixels = 0;
      for (vecText::const_iterator pos = textToRender->begin(); pos != textToRender->end(); ++pos)
      {
...
float CGUIFontTTFBase::GetTextWidthInternal(vecText::const_iterator start, vecText::const_iterator end)
{
  ...
  while (start != end)
  {
    const character_t ch = *start++;
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    if (SwitchLooksPackedTextChar(ch))
    {
      const uint8_t b0 = static_cast<uint8_t>(ch & 0xFF);
      const uint8_t b1 = static_cast<uint8_t>((ch >> 8) & 0xFF);
      const uint8_t b2 = static_cast<uint8_t>((ch >> 16) & 0xFF);
      const uint8_t b3 = static_cast<uint8_t>((ch >> 24) & 0xFF);
      const uint8_t bytes[4] = {b0, b1, b2, b3};
      for (uint8_t b : bytes)
      {
        if (!b)
          continue;
        Character* c = GetCharacter(static_cast<character_t>(b));
        if (c)
          width += c->advance;
      }
      continue;
...
CGUIFontTTFBase::Character* CGUIFontTTFBase::GetCharacter(character_t chr)
{
  ...
  if (SwitchTraceGlyphs())
  {
    static unsigned int s_rawTraceCount = 0;
    if (s_rawTraceCount < 200)
    {
      const unsigned int style8 = static_cast<unsigned int>((chr >> 24) & 0xFF);
      const unsigned int color8 = static_cast<unsigned int>((chr >> 16) & 0xFF);
      CLog::Log(LOGNOTICE,
                "SWITCH_GLYPH_RAW: chr=0x%08x letter=0x%04x style3=%u style8=%u color8=%u",
                static_cast<unsigned int>(chr),
                static_cast<unsigned int>(letter),
...
bool CGUIFontTTFBase::CacheCharacter(wchar_t letter, uint32_t style, Character *ch)
{
  ...
  if (SwitchTraceGlyphs())
  {
    static unsigned int s_glyphTraceCount = 0;
    if (s_glyphTraceCount < 400)
    {
      CLog::Log(LOGNOTICE, "SWITCH_GLYPH: font={} letter=0x{:04x} style={} glyph_index={}",
                m_strFileName, static_cast<unsigned int>(letter), style, glyph_index);
      ++s_glyphTraceCount;
    }
  }
  if (glyph_index == 0 && letter >= 0x20 && letter <= 0x7e)
  {
    static unsigned int s_asciiMissingWarns = 0;
    if (s_asciiMissingWarns < 64)
    {
      CLog::Log(LOGWARNING, "SWITCH_GLYPH: missing ascii glyph font={} letter='{}' (0x{:02x})",
                m_strFileName, static_cast<char>(letter), static_cast<unsigned int>(letter));
      ++s_asciiMissingWarns;
    }
  }
