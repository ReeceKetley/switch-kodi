...
float CGUIFont::GetTextWidth( const vecText &text )
{
  ...
  const float scaleX = CServiceBroker::GetWinSystem()->GetGfxContext().GetGUIScaleX();
  const float internal = m_font->GetTextWidthInternal(text.begin(), text.end());
  const float result = internal * scaleX;
  if (!std::isfinite(scaleX) || !std::isfinite(internal) || !std::isfinite(result) || result < 0.0f)
  {
    static unsigned int s_badWidthWarns = 0;
    if (s_badWidthWarns < 64)
    {
      CLog::Log(LOGWARNING, "SWITCH_FONT_METRIC: bad width font={} internal={} scaleX={} result={}",
                m_strFontName, internal, scaleX, result);
      ++s_badWidthWarns;
    }
    return 0.0f;
  }
...
{
  if (!m_font) return 0;
  const float scaleY = CServiceBroker::GetWinSystem()->GetGfxContext().GetGUIScaleY();
  const float internal = m_font->GetTextHeight(m_lineSpacing, numLines);
  const float result = internal * scaleY;
  if (!std::isfinite(scaleY) || !std::isfinite(internal) || !std::isfinite(result) || result < 0.0f)
  {
    static unsigned int s_badHeightWarns = 0;
    if (s_badHeightWarns < 64)
    {
      CLog::Log(LOGWARNING, "SWITCH_FONT_METRIC: bad height font={} internal={} scaleY={} result={} lines={}",
                m_strFontName, internal, scaleY, result, numLines);
      ++s_badHeightWarns;
    }
    return 0.0f;
  }
...
float CGUIFont::GetLineHeight() const
{
  if (!m_font) return 0;
  const float scaleY = CServiceBroker::GetWinSystem()->GetGfxContext().GetGUIScaleY();
  const float internal = m_font->GetLineHeight(m_lineSpacing);
  const float result = internal * scaleY;
  if (!std::isfinite(scaleY) || !std::isfinite(internal) || !std::isfinite(result) || result < 0.0f)
  {
    static unsigned int s_badLineHeightWarns = 0;
    if (s_badLineHeightWarns < 64)
    {
      CLog::Log(LOGWARNING, "SWITCH_FONT_METRIC: bad line height font={} internal={} scaleY={} result={}",
                m_strFontName, internal, scaleY, result);
      ++s_badLineHeightWarns;
    }
    return 0.0f;
  }
  return result;
