...
void CGUIControlGroupList::Render()
{
  ...
  static uint64_t s_switchGroupListRenderCalls = 0;
  ++s_switchGroupListRenderCalls;
  const bool traceGroupList = (s_switchGroupListRenderCalls <= 12);
  if (traceGroupList)
  {
    CLog::Log(LOGNOTICE, "SWITCH_GRPLIST_RENDER: begin call={} id={} parent={} children={} pos=({}, {}) size=({}, {})",
              static_cast<unsigned long long>(s_switchGroupListRenderCalls), GetID(), GetParentID(),
              static_cast<int>(m_children.size()), m_posX, m_posY, m_width, m_height);
  }
#endif
  // we run through the controls, rendering as we go
  ...
  for (iControls it = m_children.begin(); it != m_children.end(); ++it)
  {
    ...
    else
    {
      if (!std::isfinite(pos))
      {
        CLog::Log(LOGWARNING, "SWITCH_GRPLIST: non-finite pos corrected id={} pos={}", GetID(), pos);
        pos = 0.0f;
      }

      const float scroll = m_scroller.GetValue();
      float originX = m_posX;
      float originY = m_posY;
      if (m_orientation == VERTICAL)
      {
        originY = m_posY + pos - scroll;
      }
      else
      {
        originX = m_posX + pos - scroll;
      }

      if (!std::isfinite(originX) || !std::isfinite(originY))
      {
        CLog::Log(LOGWARNING,
                  "SWITCH_GRPLIST: non-finite origin corrected id={} origin=({}, {}) pos={} scroll={}",
                  GetID(), originX, originY, pos, scroll);
        originX = m_posX;
        originY = m_posY;
      }
      ...
      {
        CLog::Log(LOGNOTICE,
                  "SWITCH_GRPLIST_RENDER: child begin id={} child={} type={} pos={} scroll={}",
                  GetID(), control->GetID(), control->GetControlType(), pos, scroll);
      }
      CServiceBroker::GetWinSystem()->GetGfxContext().SetOrigin(originX, originY);
      control->DoRender();
      if (traceGroupList)
      {
        CLog::Log(LOGNOTICE,
                  "SWITCH_GRPLIST_RENDER: child done id={} child={} type={}",
                  GetID(), control->GetID(), control->GetControlType());
      }
    }
    if (control->IsVisible())
    {
      float controlSize = Size(control);
      if (!std::isfinite(controlSize) || controlSize < 0.0f)
      {
        CLog::Log(LOGWARNING, "SWITCH_GRPLIST: non-finite control size corrected id={} child={} size={}",
                  GetID(), control->GetID(), controlSize);
        controlSize = 0.0f;
      }
      pos += controlSize + m_itemGap;
    }
    CServiceBroker::GetWinSystem()->GetGfxContext().RestoreOrigin();
  }
  if (focusedControl)
  {
    if (!std::isfinite(focusedPos))
    {
      CLog::Log(LOGWARNING, "SWITCH_GRPLIST: non-finite focusedPos corrected id={} focusedPos={}", GetID(), focusedPos);
      focusedPos = 0.0f;
    }
    const float scroll = m_scroller.GetValue();
    float originX = m_posX;
    ...
    if (m_orientation == VERTICAL)
    {
      originY = m_posY + focusedPos - scroll;
    }
    else
    {
      originX = m_posX + focusedPos - scroll;
    }
    if (!std::isfinite(originX) || !std::isfinite(originY))
    {
      CLog::Log(LOGWARNING,
                "SWITCH_GRPLIST: non-finite focused origin corrected id={} origin=({}, {}) focusedPos={} scroll={}",
                GetID(), originX, originY, focusedPos, scroll);
      originX = m_posX;
      originY = m_posY;
    }
    CServiceBroker::GetWinSystem()->GetGfxContext().SetOrigin(originX, originY);
    if (traceGroupList)
      CLog::Log(LOGNOTICE, "SWITCH_GRPLIST_RENDER: focused begin id={} child={}", GetID(), focusedControl->GetID());
    focusedControl->DoRender();
    if (traceGroupList)
      CLog::Log(LOGNOTICE, "SWITCH_GRPLIST_RENDER: focused done id={} child={}", GetID(), focusedControl->GetID());
  }
  if (render) CServiceBroker::GetWinSystem()->GetGfxContext().RestoreClipRegion();
  CGUIControl::Render();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceGroupList)
    CLog::Log(LOGNOTICE, "SWITCH_GRPLIST_RENDER: done id={}", GetID());
#endif
}
