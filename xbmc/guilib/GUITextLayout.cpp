...
namespace
{
bool IsAsciiishSwitchByte(uint8_t b)
{
  return b == 0 || b == '\n' || b == '\r' || b == '\t' || (b >= 0x20 && b <= 0x7e);
}

void NormalizeSwitchPackedVecText(vecText& text)
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  bool hasPacked = false;
  for (character_t ch : text)
  {
    uint8_t b0 = static_cast<uint8_t>(ch & 0xFF);
    uint8_t b1 = static_cast<uint8_t>((ch >> 8) & 0xFF);
    uint8_t b2 = static_cast<uint8_t>((ch >> 16) & 0xFF);
    uint8_t b3 = static_cast<uint8_t>((ch >> 24) & 0xFF);
    if (IsAsciiishSwitchByte(b0) && IsAsciiishSwitchByte(b1) &&
        IsAsciiishSwitchByte(b2) && IsAsciiishSwitchByte(b3) &&
        (b2 >= 0x20 || b3 >= 0x20))
    {
      hasPacked = true;
      break;
  ...
  for (character_t ch : text)
  {
    ...
    if (IsAsciiishSwitchByte(b0) && IsAsciiishSwitchByte(b1) &&
        ...
        (b2 >= 0x20 || b3 >= 0x20))
    {
      if (b0) normalized.push_back(static_cast<character_t>(b0));
      if (b1) normalized.push_back(static_cast<character_t>(b1));
      if (b2) normalized.push_back(static_cast<character_t>(b2));
      if (b3) normalized.push_back(static_cast<character_t>(b3));
    }
    else
    {
...
void CGUITextLayout::UpdateCommon(const std::wstring &text, float maxWidth, bool forceLTRReadingOrder)
{
  ...
  NormalizeSwitchPackedVecText(parsedText);
