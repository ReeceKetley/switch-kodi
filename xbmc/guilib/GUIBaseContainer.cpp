...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
bool SwitchBaseContainerCfgContainsInt(const char* key, int value)
{
  if (!key || !*key)
    return false;
...
#else
inline bool SwitchBaseContainerCfgContainsInt(const char* key, int value) { return false; }
#endif
}
...
void CGUIBaseContainer::Render()
{
  ...
  {
    CLog::Log(LOGNOTICE, "SWITCH_CONT_RENDER: enter id={} call={} items={} cursor={} offset={} scroller={}",
              GetID(), static_cast<unsigned long long>(s_switchContainerRenderCalls),
              static_cast<int>(m_items.size()), GetCursor(), GetOffset(), m_scroller.GetValue());
  }
#endif
  ...
  if (CServiceBroker::GetWinSystem()->GetGfxContext().SetClipRegion(m_posX, m_posY, m_width, m_height))
  {
    ...
    while (pos < end && m_items.size())
    {
      ...
      if (itemNo >= 0)
      {
        ...
        if (traceContainerCall)
        {
          CLog::Log(LOGNOTICE, "SWITCH_CONT_RENDER: item begin id={} current={} itemNo={} focused={} pos={}",
                    GetID(), current, itemNo, focused ? 1 : 0, pos);
        }
#endif
        // render our item
        if (focused)
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
        if (traceContainerCall)
          CLog::Log(LOGNOTICE, "SWITCH_CONT_RENDER: item done id={} itemNo={}", GetID(), itemNo);
#endif
      }
      // increment our position
      pos += focused ? m_focusedLayout->Size(m_orientation) : m_layout->Size(m_orientation);
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceContainerCall)
        CLog::Log(LOGNOTICE, "SWITCH_CONT_RENDER: focused begin id={} pos={}", GetID(), focusedPos);
#endif
      if (m_orientation == VERTICAL)
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceContainerCall)
        CLog::Log(LOGNOTICE, "SWITCH_CONT_RENDER: focused done id={}", GetID());
#endif
    }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceContainerCall)
    CLog::Log(LOGNOTICE, "SWITCH_CONT_RENDER: done id={}", GetID());
#endif
}
...
void CGUIBaseContainer::RenderItem(float posX, float posY, CGUIListItem *item, bool focused)
{
  ...
  if (traceItem)
  {
    CLog::Log(LOGNOTICE, "SWITCH_CONT_ITEM: begin id={} focused={} pos=({}, {}) item=0x{:x}",
              GetID(), focused ? 1 : 0, posX, posY,
              static_cast<unsigned long long>(reinterpret_cast<uintptr_t>(item)));
  }
#endif
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceItem)
        CLog::Log(LOGNOTICE, "SWITCH_CONT_ITEM: focused layout render begin id={} skipFocused={}", GetID(), skipFocusedLayout ? 1 : 0);
      if (!skipFocusedLayout)
#endif
      item->GetFocusedLayout()->Render(item, m_parentID);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceItem)
        CLog::Log(LOGNOTICE, "SWITCH_CONT_ITEM: focused layout render done id={}", GetID());
#endif
    }
  }
  else
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceItem)
        CLog::Log(LOGNOTICE, "SWITCH_CONT_ITEM: unfocus anim render begin id={}", GetID());
#endif
      item->GetFocusedLayout()->Render(item, m_parentID);
    }
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceItem)
        CLog::Log(LOGNOTICE, "SWITCH_CONT_ITEM: layout render begin id={} skipLayout={}", GetID(), skipItemLayout ? 1 : 0);
      if (!skipItemLayout)
#endif
        item->GetLayout()->Render(item, m_parentID);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
      if (traceItem)
        CLog::Log(LOGNOTICE, "SWITCH_CONT_ITEM: layout render done id={}", GetID());
#endif
    }
  }
  CServiceBroker::GetWinSystem()->GetGfxContext().RestoreOrigin();
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  if (traceItem)
    CLog::Log(LOGNOTICE, "SWITCH_CONT_ITEM: done id={}", GetID());
#endif
}
