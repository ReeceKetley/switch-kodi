...
void CGraphicContext::GetGUIScaling(const RESOLUTION_INFO &res, float &scaleX, float &scaleY, TransformMatrix *matrix /* = NULL */)
{
  if (m_Resolution != RES_INVALID)
  {
    ...
    RESOLUTION_INFO info = GetResInfo();
    if (!std::isfinite(static_cast<float>(info.Overscan.left)) ||
        !std::isfinite(static_cast<float>(info.Overscan.top)) ||
        !std::isfinite(static_cast<float>(info.Overscan.right)) ||
        !std::isfinite(static_cast<float>(info.Overscan.bottom)) ||
        info.Overscan.right <= info.Overscan.left ||
        info.Overscan.bottom <= info.Overscan.top)
    {
      static unsigned int s_badOverscanWarns = 0;
      if (s_badOverscanWarns < 8)
      {
        CLog::Log(LOGWARNING,
                  "SWITCH_GUISCALE: normalizing bad overscan ({}, {}, {}, {}) to full res {}x{}",
                  info.Overscan.left, info.Overscan.top, info.Overscan.right, info.Overscan.bottom,
                  info.iWidth, info.iHeight);
        ++s_badOverscanWarns;
      }
    ...
    if (!std::isfinite(info.fPixelRatio) || info.fPixelRatio == 0.0f)
    {
      static unsigned int s_badPixelRatioWarns = 0;
      if (s_badPixelRatioWarns < 8)
      {
        CLog::Log(LOGWARNING, "SWITCH_GUISCALE: bad pixel ratio={}, forcing 1.0", info.fPixelRatio);
        ++s_badPixelRatioWarns;
      }
    ...
    if (!std::isfinite(fFromWidth) || fFromWidth <= 0.0f ||
        !std::isfinite(fFromHeight) || fFromHeight <= 0.0f ||
        !std::isfinite(fToWidth) || fToWidth <= 0.0f ||
        !std::isfinite(fToHeight) || fToHeight <= 0.0f)
    {
      static unsigned int s_badScaleInputWarns = 0;
      if (s_badScaleInputWarns < 16)
      {
        CLog::Log(LOGWARNING,
                  "SWITCH_GUISCALE: invalid inputs from=({}, {}) to=({}, {}) pos=({}, {}) overscan=({}, {}, {}, {}) insets=({}, {}, {}, {}) res={}x{}",
                  fFromWidth, fFromHeight, fToWidth, fToHeight, fToPosX, fToPosY,
                  info.Overscan.left, info.Overscan.top, info.Overscan.right, info.Overscan.bottom,
                  info.guiInsets.left, info.guiInsets.top, info.guiInsets.right, info.guiInsets.bottom,
                  info.iWidth, info.iHeight);
        ++s_badScaleInputWarns;
      }
      if (!std::isfinite(fToWidth) || fToWidth <= 0.0f)
        fToWidth = (std::isfinite(fFromWidth) && fFromWidth > 0.0f) ? fFromWidth : 1.0f;
      if (!std::isfinite(fToHeight) || fToHeight <= 0.0f)
        fToHeight = (std::isfinite(fFromHeight) && fFromHeight > 0.0f) ? fFromHeight : 1.0f;
      if (!std::isfinite(fFromWidth) || fFromWidth <= 0.0f)
        fFromWidth = 1.0f;
      if (!std::isfinite(fFromHeight) || fFromHeight <= 0.0f)
        fFromHeight = 1.0f;
    }
...
void CGraphicContext::Flip(bool rendered, bool videoLayer)
{
  ...
  CLog::Log(LOGNOTICE, "SWITCH_FLIP: enter rendered={} videoLayer={} stereo={} nextStereo={}",
            rendered ? 1 : 0, videoLayer ? 1 : 0, static_cast<int>(m_stereoMode), static_cast<int>(m_nextStereoMode));
#endif
  CServiceBroker::GetRenderSystem()->PresentRender(rendered, videoLayer);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  CLog::Log(LOGNOTICE, "SWITCH_FLIP: present done");
#endif

  if(m_stereoMode != m_nextStereoMode)
  {
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    CLog::Log(LOGNOTICE, "SWITCH_FLIP: stereo transition begin {} -> {}",
              static_cast<int>(m_stereoMode), static_cast<int>(m_nextStereoMode));
#endif
    m_stereoMode = m_nextStereoMode;
    SetVideoResolution(GetVideoResolution(), true);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    CLog::Log(LOGNOTICE, "SWITCH_FLIP: SetVideoResolution done");
#endif
    CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_NOTIFY_ALL, 0, 0, GUI_MSG_RENDERER_RESET);
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
    CLog::Log(LOGNOTICE, "SWITCH_FLIP: renderer reset msg sent");
#endif
  }
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  CLog::Log(LOGNOTICE, "SWITCH_FLIP: return");
#endif
}
