...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
static void SwitchHomeTrace(const char* fmt, ...)
{
  const char* paths[] = {
...
static bool SwitchHomeCfgHas(const char* key)
{
  if (!key)
    return false;
...
#else
static inline void SwitchHomeTrace(const char* fmt, ...) {}
static inline bool SwitchHomeCfgHas(const char* key) { return false; }
#endif
...
{
  SwitchHomeTrace("SWITCH_HOME: OnInitWindow begin");
  CLog::Log(LOGNOTICE, "SWITCH_HOME: OnInitWindow begin");
  ...
  if (StringUtils::EqualsNoCase(CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.type, "mysql") ||
    ...
    m_updateRA = (Audio | Video | Totals);
  if (SwitchHomeCfgHas("skip_home_recently_added=1"))
  {
    SwitchHomeTrace("SWITCH_HOME: AddRecentlyAddedJobs skipped by cfg");
    CLog::Log(LOGNOTICE, "SWITCH_HOME: AddRecentlyAddedJobs skipped by cfg");
  }
  else
  {
    SwitchHomeTrace("SWITCH_HOME: AddRecentlyAddedJobs begin flag=%d", m_updateRA);
    CLog::Log(LOGNOTICE, "SWITCH_HOME: AddRecentlyAddedJobs begin flag={}", m_updateRA);
    AddRecentlyAddedJobs(m_updateRA);
    SwitchHomeTrace("SWITCH_HOME: AddRecentlyAddedJobs done");
    CLog::Log(LOGNOTICE, "SWITCH_HOME: AddRecentlyAddedJobs done");
  }

  SwitchHomeTrace("SWITCH_HOME: base OnInitWindow begin");
  CLog::Log(LOGNOTICE, "SWITCH_HOME: base OnInitWindow begin");
  CGUIWindow::OnInitWindow();
  SwitchHomeTrace("SWITCH_HOME: OnInitWindow done");
  CLog::Log(LOGNOTICE, "SWITCH_HOME: OnInitWindow done");
}
...
{
  SwitchHomeTrace("SWITCH_HOME: OnMessage begin msg=%d", message.GetMessage());
  CLog::Log(LOGNOTICE, "SWITCH_HOME: OnMessage begin msg={}", message.GetMessage());
  ...
  SwitchHomeTrace("SWITCH_HOME: base OnMessage begin msg=%d", message.GetMessage());
  CLog::Log(LOGNOTICE, "SWITCH_HOME: base OnMessage begin msg={}", message.GetMessage());
  const bool handled = CGUIWindow::OnMessage(message);
  SwitchHomeTrace("SWITCH_HOME: OnMessage done msg=%d handled=%d", message.GetMessage(), handled ? 1 : 0);
  CLog::Log(LOGNOTICE, "SWITCH_HOME: OnMessage done msg={} handled={}", message.GetMessage(), handled ? 1 : 0);
  return handled;
}
