...
static int kodi_switch_uname(struct utsname* un)
{
  if (un == nullptr)
    return -1;
  std::strncpy(un->sysname, "Horizon", sizeof(un->sysname) - 1);
  std::strncpy(un->nodename, "switch", sizeof(un->nodename) - 1);
  std::strncpy(un->release, "0.0", sizeof(un->release) - 1);
  std::strncpy(un->version, "libnx", sizeof(un->version) - 1);
  std::strncpy(un->machine, "aarch64", sizeof(un->machine) - 1);
  un->sysname[sizeof(un->sysname) - 1] = '\0';
  un->nodename[sizeof(un->nodename) - 1] = '\0';
  un->release[sizeof(un->release) - 1] = '\0';
  un->version[sizeof(un->version) - 1] = '\0';
  un->machine[sizeof(un->machine) - 1] = '\0';
  return 0;
}

#define uname kodi_switch_uname
#else
...
#include <sys/param.h>
#elif defined(TARGET_LINUX) && !defined(TARGET_SWITCH)
...
#if defined(TARGET_LINUX) && !defined(TARGET_ANDROID) && !defined(TARGET_SWITCH)
