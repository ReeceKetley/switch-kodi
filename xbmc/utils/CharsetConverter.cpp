...
namespace
{
constexpr char32_t SWITCH_REPLACEMENT_CHAR = 0xFFFD;

static bool SwitchDecodeUtf8(const std::string& utf8StringSrc, std::u32string& utf32StringDst, bool failOnBadChar)
{
  utf32StringDst.clear();
  utf32StringDst.reserve(utf8StringSrc.size());

  size_t i = 0;
  while (i < utf8StringSrc.size())
  {
    const uint8_t b0 = static_cast<uint8_t>(utf8StringSrc[i]);
    char32_t cp = 0;
    size_t needed = 0;
    ...
    else if ((b0 & 0xE0) == 0xC0)
    {
      ...
      if (cp < 0x2)
      {
        if (failOnBadChar)
          return false;
        utf32StringDst.push_back(SWITCH_REPLACEMENT_CHAR);
        ++i;
        continue;
      }
    }
    ...
    if (i + needed > utf8StringSrc.size())
    {
      if (failOnBadChar)
        return false;
      utf32StringDst.push_back(SWITCH_REPLACEMENT_CHAR);
    ...
    {
      const uint8_t bj = static_cast<uint8_t>(utf8StringSrc[i + j]);
      if ((bj & 0xC0) != 0x80)
      {
        badSeq = true;
        break;
...
static bool SwitchEncodeUtf8(const std::u32string& utf32StringSrc, std::string& utf8StringDst, bool failOnBadChar)
{
  utf8StringDst.clear();
  utf8StringDst.reserve(utf32StringSrc.size() * 4);

  for (char32_t cp : utf32StringSrc)
  {
    if ((cp >= 0xD800 && cp <= 0xDFFF) || cp > 0x10FFFF)
    {
      if (failOnBadChar)
        return false;
      cp = SWITCH_REPLACEMENT_CHAR;
    }

    if (cp < 0x80)
    {
      utf8StringDst.push_back(static_cast<char>(cp));
    }
    else if (cp < 0x800)
    {
      utf8StringDst.push_back(static_cast<char>(0xC0 | (cp >> 6)));
      utf8StringDst.push_back(static_cast<char>(0x80 | (cp & 0x3F)));
    }
    else if (cp < 0x10000)
    {
      utf8StringDst.push_back(static_cast<char>(0xE0 | (cp >> 12)));
      utf8StringDst.push_back(static_cast<char>(0x80 | ((cp >> 6) & 0x3F)));
      utf8StringDst.push_back(static_cast<char>(0x80 | (cp & 0x3F)));
    }
    else
    {
      utf8StringDst.push_back(static_cast<char>(0xF0 | (cp >> 18)));
      utf8StringDst.push_back(static_cast<char>(0x80 | ((cp >> 12) & 0x3F)));
      utf8StringDst.push_back(static_cast<char>(0x80 | ((cp >> 6) & 0x3F)));
      utf8StringDst.push_back(static_cast<char>(0x80 | (cp & 0x3F)));
    }
  }
  ...
  #define UTF8_SOURCE "UTF-8"
  #define WCHAR_CHARSET UTF32_CHARSET
#elif defined(TARGET_SWITCH) || defined(__SWITCH__)
  #define WCHAR_IS_UCS_4 1
  #define UTF16_CHARSET "UTF-16" ENDIAN_SUFFIX
  #define UTF32_CHARSET "UTF-32" ENDIAN_SUFFIX
  #define UTF8_SOURCE "UTF-8"
...
{
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  // On Switch, bypass iconv for UTF-8<->wstring conversion to avoid packed-byte corruption.
  std::u32string utf32str;
  if (!SwitchDecodeUtf8(utf8StringSrc, utf32str, failOnBadChar))
    return false;

  if (bVisualBiDiFlip)
  {
    std::u32string utf32flipped;
    const bool bidiResult =
      CInnerConverter::logicalToVisualBiDi(utf32str, utf32flipped,
                                            forceLTRReadingOrder ? FRIBIDI_TYPE_LTR : FRIBIDI_TYPE_PDF,
                                            failOnBadChar);
    return utf32ToW(utf32flipped, wStringDst, failOnBadChar) && bidiResult;
  }

  return utf32ToW(utf32str, wStringDst, failOnBadChar);
#else
...
#if defined(TARGET_SWITCH) || defined(__SWITCH__)
  std::u32string utf32;
  utf32.reserve(wStringSrc.size());
  for (wchar_t wc : wStringSrc)
    utf32.push_back(static_cast<char32_t>(wc));
  return SwitchEncodeUtf8(utf32, utf8StringDst, failOnBadChar);
#else
