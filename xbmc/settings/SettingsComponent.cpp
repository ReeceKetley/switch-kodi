...
#if defined(TARGET_SWITCH)
static void SwitchSettingsTrace(const char* fmt, ...)
{
  const char* paths[] = {
...
#else
static inline void SwitchSettingsTrace(const char* fmt, ...) {}
#endif
...
{
  SwitchSettingsTrace("Settings::Init enter");
  if (m_state == State::DEINITED)
  {
    SwitchSettingsTrace("Settings::Init state deinit");
    // only the InitDirectories* for the current platform should return true
    bool inited = InitDirectoriesLinux(params.m_platformDirectories);
    SwitchSettingsTrace("Settings::Init after InitDirectoriesLinux");
    if (!inited)
      inited = InitDirectoriesOSX(params.m_platformDirectories);
    SwitchSettingsTrace("Settings::Init after InitDirectoriesOSX");
    if (!inited)
      inited = InitDirectoriesWin32(params.m_platformDirectories);
    SwitchSettingsTrace("Settings::Init after InitDirectoriesWin32");

    m_settings->Initialize();
    SwitchSettingsTrace("Settings::Init m_settings initialized");

    m_advancedSettings->Initialize(params, *m_settings->GetSettingsManager());
    SwitchSettingsTrace("Settings::Init advanced settings initialized");
    URIUtils::RegisterAdvancedSettings(*m_advancedSettings);
    SwitchSettingsTrace("Settings::Init advanced settings registered");

    m_profileManager->Initialize(m_settings);
    SwitchSettingsTrace("Settings::Init profile manager initialized");

    CServiceBroker::RegisterSettingsComponent(this);
    SwitchSettingsTrace("Settings::Init settings component registered");

    m_state = State::INITED;
    SwitchSettingsTrace("Settings::Init success");
  }
}
...
bool CSettingsComponent::InitDirectoriesLinux(bool bPlatformDirectories)
{
  ...
  SwitchSettingsTrace("Settings::InitDirsLinux enter");
  std::string appPath;
  std::string appName = CCompileInfo::GetAppName();
  SwitchSettingsTrace("Settings::InitDirsLinux appName=%s", appName.c_str());
  ...
  SwitchSettingsTrace("Settings::InitDirsLinux appBinPath=%s", appBinPath.c_str());
#if defined(TARGET_SWITCH)
  // Keep Switch path deterministic for current port bring-up.
  appPath = "sdmc:/switch/kodi";
  appBinPath = appPath;
  SwitchSettingsTrace("Settings::InitDirsLinux switch appPath=%s", appPath.c_str());
#else
  // overridden by user
  ...
  else
  {
    ...
    if (!XFILE::CDirectory::Exists(URIUtils::AddFileToFolder(appPath, "userdata")))
    {
      ...
      SwitchSettingsTrace("Settings::InitDirsLinux fallback appPath=%s", appPath.c_str());
      if (!XFILE::CDirectory::Exists(URIUtils::AddFileToFolder(appPath, "userdata")))
      {
        fprintf(stderr, "Unable to find path to %s data files!\n", appName.c_str());
        SwitchSettingsTrace("Settings::InitDirsLinux userdata lookup failed");
        exit(1);
      }
    }
  }
...
#if defined(TARGET_SWITCH)
  setenv(envAppBinHome, appBinPath.c_str(), 1);
  setenv(envAppHome, appPath.c_str(), 1);
#else
  setenv(envAppBinHome, appBinPath.c_str(), 0);
  setenv(envAppHome, appPath.c_str(), 0);
#endif
  SwitchSettingsTrace("Settings::InitDirsLinux env set");
  ...
  if (bPlatformDirectories)
  {
    ...
    CreateUserDirs();
    SwitchSettingsTrace("Settings::InitDirsLinux platform dirs created");
  ...
  else
  {
    ...
    CSpecialProtocol::SetLogPath(strTempPath);
    CreateUserDirs();
    SwitchSettingsTrace("Settings::InitDirsLinux portable dirs created");
  }
  CSpecialProtocol::SetXBMCBinAddonPath(appBinPath + "/addons");
  SwitchSettingsTrace("Settings::InitDirsLinux done");
...
void CSettingsComponent::CreateUserDirs() const
{
  ...
  XFILE::CDirectory::Create("special://masterprofile/Database");
  XFILE::CDirectory::Create("special://masterprofile/Thumbnails");
  XFILE::CDirectory::Create("special://masterprofile/addon_data");
