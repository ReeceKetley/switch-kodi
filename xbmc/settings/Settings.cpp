...
bool CSettings::Load(const std::string &file)
{
      ...
      !LoadValuesFromXml(xmlDoc, updated))
  {
    ...
    {
      CLog::Log(LOGWARNING, "CSettings: Reset() could not persist defaults on Switch, continuing with in-memory defaults");
      return true;
    }
#else
      return false;
...
#if defined(TARGET_SWITCH)
    // Switch homebrew may fail early writeback; keep defaults in memory and continue.
    return true;
#else
    if (!Load(file))
      return false;
