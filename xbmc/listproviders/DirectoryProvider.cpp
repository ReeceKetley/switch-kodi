   ...
   m_currentLimit(0)
{
  ...
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider ctor enter");
  assert(element);
  if (!element->NoChildren())
  {
    ...
    {
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider target begin");
      m_target.SetLabel(target, "", parentID);
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider target done");
    }
    ...
    {
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider sortby begin");
      m_sortMethod.SetLabel(sortMethod, "", parentID);
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider sortby done");
    }
    ...
    {
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider sortorder begin");
      m_sortOrder.SetLabel(sortOrder, "", parentID);
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider sortorder done");
    }
    ...
    {
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider limit begin");
      m_limit.SetLabel(limit, "", parentID);
      if (isHome)
        CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider limit done");
    }

    if (isHome)
      CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider url begin");
    m_url.SetLabel(element->FirstChild()->ValueStr(), "", parentID);
    if (isHome)
      CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider url done");
  }
  if (isHome)
    CLog::Log(LOGNOTICE, "SWITCH_PROVIDER: DirectoryProvider ctor done");
}
