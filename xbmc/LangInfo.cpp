...
bool CLangInfo::Load(const std::string& strLanguage)
{
  ...
  if (!xmlDoc.LoadFile(strFileName))
  {
    ...
    const std::string addonId = ADDON::CLanguageResource::GetAddonId(strLanguage);
    const std::string fallbackPath =
        std::string("special://home/addons/") + addonId + "/resources/langinfo.xml";
    CLog::Log(LOGWARNING,
              "CLangInfo: primary langinfo load failed at '%s' (%s line %d), trying fallback '%s'",
              strFileName.c_str(),
              xmlDoc.ErrorDesc(),
              xmlDoc.ErrorRow(),
              fallbackPath.c_str());

    if (!xmlDoc.LoadFile(fallbackPath))
    {
      CLog::Log(LOGERROR, "unable to load %s: %s at line %d", fallbackPath.c_str(), xmlDoc.ErrorDesc(), xmlDoc.ErrorRow());
      return false;
    }
    strFileName = fallbackPath;
#else
...
std::string CLangInfo::GetLanguageInfoPath(const std::string &language)
{
  ...
  return std::string("special://home/addons/") + addonId + "/resources/langinfo.xml";
#else
