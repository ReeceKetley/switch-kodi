...
#include <limits.h>
#if defined(TARGET_ANDROID) || defined(TARGET_SWITCH)
#include <unistd.h>
#else
...
#if !defined(TARGET_ANDROID) && !defined(TARGET_SWITCH) // http://code.google.com/p/android/issues/detail?id=7808
...
#if defined(RLIMIT_NICE) && !defined(TARGET_SWITCH)
  // get user max prio
  struct rlimit limit;
  int userMaxPrio;
