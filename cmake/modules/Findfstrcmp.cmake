...
if(CORE_PLATFORM_NAME STREQUAL "switch")
  set(FSTRCMP_INCLUDE_DIR ${CMAKE_SOURCE_DIR}/cmake/platform/switch)
  set(FSTRCMP_LIBRARY fstrcmp)

  if(NOT TARGET fstrcmp)
    add_library(fstrcmp STATIC ${CMAKE_SOURCE_DIR}/cmake/platform/switch/fstrcmp_stub.c)
    target_include_directories(fstrcmp PUBLIC ${FSTRCMP_INCLUDE_DIR})
    set_target_properties(fstrcmp PROPERTIES FOLDER "External Projects")
  endif()

  include(FindPackageHandleStandardArgs)
  find_package_handle_standard_args(fstrcmp
                                    REQUIRED_VARS FSTRCMP_INCLUDE_DIR FSTRCMP_LIBRARY)

  set(FSTRCMP_LIBRARIES ${FSTRCMP_LIBRARY})
  set(FSTRCMP_INCLUDE_DIRS ${FSTRCMP_INCLUDE_DIR})
elseif(ENABLE_INTERNAL_FSTRCMP)
  ...
  set(FSTRCMP_HOST_ARG "")
  if(CMAKE_C_COMPILER_TARGET)
    set(FSTRCMP_HOST_ARG "--host=${CMAKE_C_COMPILER_TARGET}")
  elseif(CORE_SYSTEM_NAME STREQUAL linux AND CMAKE_SYSTEM_PROCESSOR STREQUAL aarch64)
    set(FSTRCMP_HOST_ARG "--host=aarch64-none-elf")
  endif()
  set(FSTRCMP_CONFIGURE_COMMAND
      ${CMAKE_COMMAND} -E env
      CC=${CMAKE_C_COMPILER}
      ...
      RANLIB=${CMAKE_RANLIB}
      STRIP=${CMAKE_STRIP}
      sh -c "autoreconf -vif && ./configure ${FSTRCMP_HOST_ARG} --prefix ${CMAKE_BINARY_DIR}/${CORE_BUILD_DIR}")
  externalproject_add(fstrcmp
                      URL ${FSTRCMP_URL}
                      DOWNLOAD_DIR ${CMAKE_BINARY_DIR}/${CORE_BUILD_DIR}/download
                      PREFIX ${CORE_BUILD_DIR}/fstrcmp
                      CONFIGURE_COMMAND ${FSTRCMP_CONFIGURE_COMMAND}
                      BUILD_COMMAND make lib/libfstrcmp.la
                      INSTALL_COMMAND make install-include install-libdir
